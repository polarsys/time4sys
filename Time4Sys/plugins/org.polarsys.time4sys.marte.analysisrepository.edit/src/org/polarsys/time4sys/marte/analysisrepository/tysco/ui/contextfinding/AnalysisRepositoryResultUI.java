...
public class AnalysisRepositoryResultUI extends TitleAreaDialog {
	...
	private Composite compoTab1, compoTab2, compoTab3, compoTab4;
	...
	protected Control createDialogArea(Composite parent) {
		...
		matchedContextsText = new Text(matchedContextsGroup,SWT.MULTI | SWT.BORDER | SWT.WRAP | SWT.V_SCROLL| SWT.READ_ONLY);
		...
		//ScrolledComposite scroll = new ScrolledComposite(tabFolder,SWT.V_SCROLL|SWT.H_SCROLL|SWT.BORDER);
		...
		//scroll = new ScrolledComposite(tabFolder,SWT.V_SCROLL|SWT.H_SCROLL|SWT.BORDER);
		scroll = new ScrolledComposite(tabFolder,SWT.V_SCROLL|SWT.H_SCROLL);
		scroll.setExpandHorizontal(true);
		scroll.setExpandVertical(true);
		//grid = new GridData(GridData.FILL_BOTH|GridData.GRAB_HORIZONTAL|GridData.GRAB_VERTICAL);
		grid = new GridData(SWT.FILL,SWT.FILL,false,true);
		scroll.setLayoutData(grid);
...
/************************** TAB2 **********************************/		
	...
	public void addNotAcceptedContextModelDetail(ContextModel ctx,String matchedPercent) {
		...
		String ctxName = ctx.getName();
		String description = ctx.getDescription();
		//String matchedPercent = computeMatchedPercent(ctx.getRootGroup(),results);
	...
	/*private float computeMatchedPercent(RuleGroup rootGroup,List<Result> results) {
		
		float matchedPercent = 0;
		
		if (rootGroup.getConjunctionType() == ConjunctionType.AND) {
			int numberOfRules = rootGroup.getExpectedRuleValues().size();
			int numberOfSubGroups = rootGroup.getSubGroups().size();
			
			int numberOfSatisfiedRules = 0;
			for ()
		}
...
/************************** TAB4 **********************************/	
			...
			for (TestCharacteristicType characType : characteristics) {
				...
				if (!associatedContexts.isEmpty()) {
					for (ContextModel ctx : associatedContexts) {
						if (ctrl.isAccepted(ctx)) mess += ctx.getName()+", ";
					}
				}
	...
	private String getCharacteristicName(TestCharacteristicType characType) {
		if (characType instanceof NecessaryCondition)  				return "Necessary";
		else if (characType instanceof SufficientCondition) 		return "Sufficient";
		else if (characType instanceof Period_Sustainability) 		return "Period Sustainable";
		else if (characType instanceof Deadline_Sustainability) 	return "Deadline Sustainable";
		else if (characType instanceof ExecutionTime_Sustainability)return "Execution-Time Sustainable";
		else if (characType instanceof Jitter_Sustainability) 		return "Jitter Sustainable";
		return "";
	}
