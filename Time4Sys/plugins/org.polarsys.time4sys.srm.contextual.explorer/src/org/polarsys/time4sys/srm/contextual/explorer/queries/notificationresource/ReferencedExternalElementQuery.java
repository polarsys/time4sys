...
package org.polarsys.time4sys.srm.contextual.explorer.queries.notificationresource;
...
public class ReferencedExternalElementQuery implements IQuery {
	...
	*/
	public List<Object> compute(Object object_p) {
		...
		NotificationResource object = (NotificationResource) object_p;

		for (EObject eObject : object.getFlushServices())
			result.add(eObject);

		for (EObject eObject : object.getSignalServices())
			result.add(eObject);

		for (EObject eObject : object.getWaitServices())
			result.add(eObject);

		for (EObject eObject : object.getClearServices())
			result.add(eObject);
