...
package org.polarsys.time4sys.srm.contextual.explorer.queries.softwareresource;
...
public class ReferencedExternalElementQuery implements IQuery {
	...
	*/
	public List<Object> compute(Object object_p) {
		...
		SoftwareResource object = (SoftwareResource) object_p;

		for (EObject eObject : object.getCreateServices())
			result.add(eObject);

		for (EObject eObject : object.getDeleteServices())
			result.add(eObject);

		for (EObject eObject : object.getInitializeServices())
			result.add(eObject);
