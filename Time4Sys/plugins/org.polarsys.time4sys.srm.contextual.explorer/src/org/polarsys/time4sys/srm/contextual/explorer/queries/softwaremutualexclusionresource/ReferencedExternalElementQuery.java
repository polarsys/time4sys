...
package org.polarsys.time4sys.srm.contextual.explorer.queries.softwaremutualexclusionresource;
...
public class ReferencedExternalElementQuery implements IQuery {
	...
	*/
	public List<Object> compute(Object object_p) {
		...
		SoftwareMutualExclusionResource object = (SoftwareMutualExclusionResource) object_p;

		for (EObject eObject : object.getAcquireServices())
			result.add(eObject);

		for (EObject eObject : object.getReleaseServices())
			result.add(eObject);

		for (EObject eObject : object.getAccessTokenElements())
			result.add(eObject);
