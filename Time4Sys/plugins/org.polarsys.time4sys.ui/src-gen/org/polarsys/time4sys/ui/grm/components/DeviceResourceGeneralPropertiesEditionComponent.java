 ...
 * 
 */
public class DeviceResourceGeneralPropertiesEditionComponent extends SinglePartPropertiesEditingComponent {
	 ...
	 * 
	 */
	public DeviceResourceGeneralPropertiesEditionComponent(PropertiesEditingContext editingContext, EObject deviceResource, String editing_mode) {
		super(editingContext, deviceResource, editing_mode);
	...
	public void initPart(Object key, int kind, EObject elt, ResourceSet allResource) {
		...
		if (editingPart != null && key == partKey) {
			...
			final DeviceResource deviceResource = (DeviceResource)elt;
			final GeneralPropertiesEditionPart generalPart = (GeneralPropertiesEditionPart)editingPart;
			// init values
			if (isAccessible(GrmViewsRepository.General.Properties.name))
				generalPart.setName(EEFConverterUtil.convertToString(EcorePackage.Literals.ESTRING, deviceResource.getName()));
			
			if (isAccessible(GrmViewsRepository.General.Properties.ownedResource)) {
				ownedResourceSettings = new ReferencesTableSettings(deviceResource, GrmPackage.eINSTANCE.getResource_OwnedResource());
				generalPart.initOwnedResource(ownedResourceSettings);
			}
			if (isAccessible(GrmViewsRepository.General.Properties.resMult)) {
				generalPart.setResMult(EEFConverterUtil.convertToString(EcorePackage.Literals.EINT, deviceResource.getResMult()));
			}
			
			if (isAccessible(GrmViewsRepository.General.Properties.isPeriodic)) {
				generalPart.setIsPeriodic(deviceResource.isIsProtected());
			}
			if (isAccessible(GrmViewsRepository.General.Properties.isActive)) {
				generalPart.setIsActive(deviceResource.isIsActive());
			}
			if (isAccessible(GrmViewsRepository.General.Properties.ownedPort)) {
				ownedPortSettings = new ReferencesTableSettings(deviceResource, GrmPackage.eINSTANCE.getResource_OwnedPort());
				generalPart.initOwnedPort(ownedPortSettings);
			}
			if (isAccessible(GrmViewsRepository.General.Properties.ownedConnector)) {
				ownedConnectorSettings = new ReferencesTableSettings(deviceResource, GrmPackage.eINSTANCE.getResource_OwnedConnector());
				generalPart.initOwnedConnector(ownedConnectorSettings);
			}
			if (isAccessible(GrmViewsRepository.General.Properties.usedBy)) {
				usedBySettings = new ReferencesTableSettings(deviceResource, GrmPackage.eINSTANCE.getResource_UsedBy());
				generalPart.initUsedBy(usedBySettings);
			}
			if (isAccessible(GrmViewsRepository.General.Properties.speedFactor)) {
				generalPart.setSpeedFactor(EEFConverterUtil.convertToString(EcorePackage.Literals.EFLOAT, deviceResource.getSpeedFactor()));
			}
			
			if (isAccessible(GrmViewsRepository.General.Properties.mainScheduler)) {
				// init part
				mainSchedulerSettings = new EObjectFlatComboSettings(deviceResource, GrmPackage.eINSTANCE.getProcessingResource_MainScheduler());
	...
	public void updateSemanticModel(final IPropertiesEditionEvent event) {
		DeviceResource deviceResource = (DeviceResource)semanticObject;
		if (GrmViewsRepository.General.Properties.name == event.getAffectedEditor()) {
			deviceResource.setName((java.lang.String)EEFConverterUtil.createFromString(EcorePackage.Literals.ESTRING, (String)event.getNewValue()));
		}
		...
		if (GrmViewsRepository.General.Properties.resMult == event.getAffectedEditor()) {
			deviceResource.setResMult((EEFConverterUtil.createIntFromString(EcorePackage.Literals.EINT, (String)event.getNewValue())));
		}
		if (GrmViewsRepository.General.Properties.isPeriodic == event.getAffectedEditor()) {
			deviceResource.setIsProtected((Boolean)event.getNewValue());
		}
		if (GrmViewsRepository.General.Properties.isActive == event.getAffectedEditor()) {
			deviceResource.setIsActive((Boolean)event.getNewValue());
		}
		...
		if (GrmViewsRepository.General.Properties.speedFactor == event.getAffectedEditor()) {
			deviceResource.setSpeedFactor((EEFConverterUtil.createFloatFromString(EcorePackage.Literals.EFLOAT, (String)event.getNewValue())));
		}
