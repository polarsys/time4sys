 ...
 * 
 */
public class SecondarySchedulerGeneralPropertiesEditionComponent extends SinglePartPropertiesEditingComponent {
	...
	/**
	 * Settings for virtualProcessingUnit ReferencesTable
	 */
	private ReferencesTableSettings virtualProcessingUnitSettings;
	 ...
	 * 
	 */
	public SecondarySchedulerGeneralPropertiesEditionComponent(PropertiesEditingContext editingContext, EObject secondaryScheduler, String editing_mode) {
		super(editingContext, secondaryScheduler, editing_mode);
	...
	public void initPart(Object key, int kind, EObject elt, ResourceSet allResource) {
		...
		if (editingPart != null && key == partKey) {
			...
			final SecondaryScheduler secondaryScheduler = (SecondaryScheduler)elt;
			final GeneralPropertiesEditionPart generalPart = (GeneralPropertiesEditionPart)editingPart;
			// init values
			if (isAccessible(GrmViewsRepository.General.Properties.name))
				generalPart.setName(EEFConverterUtil.convertToString(EcorePackage.Literals.ESTRING, secondaryScheduler.getName()));
			
			if (isAccessible(GrmViewsRepository.General.Properties.ownedResource)) {
				ownedResourceSettings = new ReferencesTableSettings(secondaryScheduler, GrmPackage.eINSTANCE.getResource_OwnedResource());
				generalPart.initOwnedResource(ownedResourceSettings);
			}
			if (isAccessible(GrmViewsRepository.General.Properties.resMult)) {
				generalPart.setResMult(EEFConverterUtil.convertToString(EcorePackage.Literals.EINT, secondaryScheduler.getResMult()));
			}
			
			if (isAccessible(GrmViewsRepository.General.Properties.isProtected)) {
				generalPart.setIsProtected(secondaryScheduler.isIsProtected());
			}
			if (isAccessible(GrmViewsRepository.General.Properties.isActive)) {
				generalPart.setIsActive(secondaryScheduler.isIsActive());
			}
			if (isAccessible(GrmViewsRepository.General.Properties.ownedPort)) {
				ownedPortSettings = new ReferencesTableSettings(secondaryScheduler, GrmPackage.eINSTANCE.getResource_OwnedPort());
				generalPart.initOwnedPort(ownedPortSettings);
			}
			if (isAccessible(GrmViewsRepository.General.Properties.ownedConnector)) {
				ownedConnectorSettings = new ReferencesTableSettings(secondaryScheduler, GrmPackage.eINSTANCE.getResource_OwnedConnector());
				generalPart.initOwnedConnector(ownedConnectorSettings);
			}
			if (isAccessible(GrmViewsRepository.General.Properties.usedBy)) {
				usedBySettings = new ReferencesTableSettings(secondaryScheduler, GrmPackage.eINSTANCE.getResource_UsedBy());
				generalPart.initUsedBy(usedBySettings);
			}
			if (isAccessible(GrmViewsRepository.General.Properties.brokedResource)) {
				brokedResourceSettings = new ReferencesTableSettings(secondaryScheduler, GrmPackage.eINSTANCE.getResourceBroker_BrokedResource());
				generalPart.initBrokedResource(brokedResourceSettings);
			}
			// FIXME NO VALID CASE INTO template public updater(editionElement : PropertiesEditionElement, view : View, pec : PropertiesEditionComponent) in widgetControl.mtl module, with the values : accCtrlPolicy, General, SecondaryScheduler.
			if (isAccessible(GrmViewsRepository.General.Properties.host)) {
				// init part
				hostSettings = new EObjectFlatComboSettings(secondaryScheduler, GrmPackage.eINSTANCE.getScheduler_Host());
			...
			if (isAccessible(GrmViewsRepository.General.Properties.processingUnits)) {
				processingUnitsSettings = new ReferencesTableSettings(secondaryScheduler, GrmPackage.eINSTANCE.getScheduler_ProcessingUnits());
			...
			if (isAccessible(GrmViewsRepository.General.Properties.schedulableResource)) {
				schedulableResourceSettings = new ReferencesTableSettings(secondaryScheduler, GrmPackage.eINSTANCE.getScheduler_SchedulableResource());
				generalPart.initSchedulableResource(schedulableResourceSettings);
			}
			if (isAccessible(GrmViewsRepository.General.Properties.virtualProcessingUnit)) {
				virtualProcessingUnitSettings = new ReferencesTableSettings(secondaryScheduler, GrmPackage.eINSTANCE.getSecondaryScheduler_VirtualProcessingUnit());
				generalPart.initVirtualProcessingUnit(virtualProcessingUnitSettings);
			}
			// init filters
			...
			// FIXME NO VALID CASE INTO template public filterUpdater(editionElement : PropertiesEditionElement, view : View, pec : PropertiesEditionComponent) in widgetControl.mtl module, with the values : accCtrlPolicy, General, SecondaryScheduler.
			if (isAccessible(GrmViewsRepository.General.Properties.host)) {
			...
			if (isAccessible(GrmViewsRepository.General.Properties.virtualProcessingUnit)) {
				generalPart.addFilterToVirtualProcessingUnit(new EObjectFilter(GrmPackage.Literals.SCHEDULABLE_RESOURCE));
				// Start of user code for additional businessfilters for virtualProcessingUnit
				// End of user code
			}
	...
	public EStructuralFeature associatedFeature(Object editorKey) {
		...
		if (editorKey == GrmViewsRepository.General.Properties.virtualProcessingUnit) {
			return GrmPackage.eINSTANCE.getSecondaryScheduler_VirtualProcessingUnit();
		}
	...
	public void updateSemanticModel(final IPropertiesEditionEvent event) {
		SecondaryScheduler secondaryScheduler = (SecondaryScheduler)semanticObject;
		if (GrmViewsRepository.General.Properties.name == event.getAffectedEditor()) {
			secondaryScheduler.setName((java.lang.String)EEFConverterUtil.createFromString(EcorePackage.Literals.ESTRING, (String)event.getNewValue()));
		}
		...
		if (GrmViewsRepository.General.Properties.resMult == event.getAffectedEditor()) {
			secondaryScheduler.setResMult((EEFConverterUtil.createIntFromString(EcorePackage.Literals.EINT, (String)event.getNewValue())));
		}
		if (GrmViewsRepository.General.Properties.isProtected == event.getAffectedEditor()) {
			secondaryScheduler.setIsProtected((Boolean)event.getNewValue());
		}
		if (GrmViewsRepository.General.Properties.isActive == event.getAffectedEditor()) {
			secondaryScheduler.setIsActive((Boolean)event.getNewValue());
		}
		...
		if (GrmViewsRepository.General.Properties.brokedResource == event.getAffectedEditor()) {
			// FIXME INVALID CASE you must override the template 'declareEObjectUpdater' for the case : accCtrlPolicy, General, SecondaryScheduler.
		}
		...
		if (GrmViewsRepository.General.Properties.virtualProcessingUnit == event.getAffectedEditor()) {
			if (event.getKind() == PropertiesEditionEvent.ADD) {
				if (event.getNewValue() instanceof SchedulableResource) {
					virtualProcessingUnitSettings.addToReference((EObject) event.getNewValue());
				}
			} else if (event.getKind() == PropertiesEditionEvent.REMOVE) {
				virtualProcessingUnitSettings.removeFromReference((EObject) event.getNewValue());
			} else if (event.getKind() == PropertiesEditionEvent.MOVE) {
				virtualProcessingUnitSettings.move(event.getNewIndex(), (SchedulableResource) event.getNewValue());
			}
		}
	}
	...
	public void updatePart(Notification msg) {
		...
		if (editingPart.isVisible()) {
			...
			// FIXME INVALID CASE INTO template public liveUpdater(editionElement : PropertiesEditionElement, view : View, pec : PropertiesEditionComponent) in widgetControl.mtl module, with the values : accCtrlPolicy, General, SecondaryScheduler.
			...
			if (GrmPackage.eINSTANCE.getSecondaryScheduler_VirtualProcessingUnit().equals(msg.getFeature())  && isAccessible(GrmViewsRepository.General.Properties.virtualProcessingUnit))
				generalPart.updateVirtualProcessingUnit();
	...
	protected NotificationFilter[] getNotificationFilters() {
		NotificationFilter filter = new EStructuralFeatureNotificationFilter(
			...
			GrmPackage.eINSTANCE.getSecondaryScheduler_VirtualProcessingUnit()		);
	...
	public boolean isRequired(Object key, int kind) {
		return key == GrmViewsRepository.General.Properties.brokedResource || key == GrmViewsRepository.General.Properties.brokedResource || key == GrmViewsRepository.General.Properties.host || key == GrmViewsRepository.General.Properties.processingUnits || key == GrmViewsRepository.General.Properties.virtualProcessingUnit;
	}
