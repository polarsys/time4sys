 ...
 * 
 */
public class ReleaseStepTimingPropertiesEditionComponent extends SinglePartPropertiesEditingComponent {
	 ...
	 * 
	 */
	public ReleaseStepTimingPropertiesEditionComponent(PropertiesEditingContext editingContext, EObject releaseStep, String editing_mode) {
	...
	public void initPart(Object key, int kind, EObject elt, ResourceSet allResource) {
		...
		if (editingPart != null && key == partKey) {
			...
			if (isAccessible(GqamViewsRepository.Timing.TimingProperties.respTime))
				timingPart.setRespTime(EEFConverterUtil.convertToString(GqamPackage.Literals.NFP_DURATION, releaseStep.getRespTime()));
			
			if (isAccessible(GqamViewsRepository.Timing.TimingProperties.bestCET))
				timingPart.setBestCET(EEFConverterUtil.convertToString(GqamPackage.Literals.NFP_DURATION, releaseStep.getBestCET()));
			
			if (isAccessible(GqamViewsRepository.Timing.TimingProperties.worstCET))
				timingPart.setWorstCET(EEFConverterUtil.convertToString(GqamPackage.Literals.NFP_DURATION, releaseStep.getWorstCET()));
			
			if (isAccessible(GqamViewsRepository.Timing.TimingProperties.blockingTime))
				timingPart.setBlockingTime(EEFConverterUtil.convertToString(GqamPackage.Literals.NFP_DURATION, releaseStep.getBlockingTime()));
	...
	public void updateSemanticModel(final IPropertiesEditionEvent event) {
		...
		if (GqamViewsRepository.Timing.TimingProperties.respTime == event.getAffectedEditor()) {
			releaseStep.setRespTime((org.polarsys.time4sys.marte.nfp.Duration)EEFConverterUtil.createFromString(GqamPackage.Literals.NFP_DURATION, (String)event.getNewValue()));
		}
		if (GqamViewsRepository.Timing.TimingProperties.bestCET == event.getAffectedEditor()) {
			releaseStep.setBestCET((org.polarsys.time4sys.marte.nfp.Duration)EEFConverterUtil.createFromString(GqamPackage.Literals.NFP_DURATION, (String)event.getNewValue()));
		}
		if (GqamViewsRepository.Timing.TimingProperties.worstCET == event.getAffectedEditor()) {
			releaseStep.setWorstCET((org.polarsys.time4sys.marte.nfp.Duration)EEFConverterUtil.createFromString(GqamPackage.Literals.NFP_DURATION, (String)event.getNewValue()));
		}
		if (GqamViewsRepository.Timing.TimingProperties.blockingTime == event.getAffectedEditor()) {
			releaseStep.setBlockingTime((org.polarsys.time4sys.marte.nfp.Duration)EEFConverterUtil.createFromString(GqamPackage.Literals.NFP_DURATION, (String)event.getNewValue()));
		}
	}
