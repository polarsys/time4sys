 ...
 * 
 */
public class SoftwareMutualExclusionResourceGeneralPropertiesEditionComponent extends SinglePartPropertiesEditingComponent {
	 ...
	 * 
	 */
	public SoftwareMutualExclusionResourceGeneralPropertiesEditionComponent(PropertiesEditingContext editingContext, EObject softwareMutualExclusionResource, String editing_mode) {
	...
	public void initPart(Object key, int kind, EObject elt, ResourceSet allResource) {
		...
		if (editingPart != null && key == partKey) {
			...
			if (isAccessible(SrmViewsRepository.General.Properties.name))
				generalPart.setName(EEFConverterUtil.convertToString(EcorePackage.Literals.ESTRING, softwareMutualExclusionResource.getName()));
			
			if (isAccessible(SrmViewsRepository.General.Properties.ownedResource)) {
				ownedResourceSettings = new ReferencesTableSettings(softwareMutualExclusionResource, GrmPackage.eINSTANCE.getResource_OwnedResource());
				generalPart.initOwnedResource(ownedResourceSettings);
			}
			if (isAccessible(SrmViewsRepository.General.Properties.resMult)) {
				generalPart.setResMult(EEFConverterUtil.convertToString(EcorePackage.Literals.EINT, softwareMutualExclusionResource.getResMult()));
			}
			
			if (isAccessible(SrmViewsRepository.General.Properties.isProtected)) {
				generalPart.setIsProtected(softwareMutualExclusionResource.isIsProtected());
			}
			if (isAccessible(SrmViewsRepository.General.Properties.isActive)) {
				generalPart.setIsActive(softwareMutualExclusionResource.isIsActive());
			}
			if (isAccessible(SrmViewsRepository.General.Properties.ownedPort)) {
				ownedPortSettings = new ReferencesTableSettings(softwareMutualExclusionResource, GrmPackage.eINSTANCE.getResource_OwnedPort());
				generalPart.initOwnedPort(ownedPortSettings);
			}
			if (isAccessible(SrmViewsRepository.General.Properties.ownedConnector)) {
				ownedConnectorSettings = new ReferencesTableSettings(softwareMutualExclusionResource, GrmPackage.eINSTANCE.getResource_OwnedConnector());
				generalPart.initOwnedConnector(ownedConnectorSettings);
			}
			if (isAccessible(SrmViewsRepository.General.Properties.usedBy)) {
				usedBySettings = new ReferencesTableSettings(softwareMutualExclusionResource, GrmPackage.eINSTANCE.getResource_UsedBy());
				generalPart.initUsedBy(usedBySettings);
			}
			if (isAccessible(SrmViewsRepository.General.Properties.managedResource)) {
				managedResourceSettings = new ReferencesTableSettings(softwareMutualExclusionResource, GrmPackage.eINSTANCE.getResourceManager_ManagedResource());
			...
			if (isAccessible(SrmViewsRepository.General.Properties.memorySizeFootprint)) {
				generalPart.setMemorySizeFootprint(EEFConverterUtil.convertToString(EcorePackage.Literals.EINT, softwareMutualExclusionResource.getMemorySizeFootprint()));
			}
			
			if (isAccessible(SrmViewsRepository.General.Properties.stateElements))
				generalPart.setStateElements(softwareMutualExclusionResource.getStateElements());
			
			if (isAccessible(SrmViewsRepository.General.Properties.identifierElements))
				generalPart.setIdentifierElements(softwareMutualExclusionResource.getIdentifierElements());
			
			if (isAccessible(SrmViewsRepository.General.Properties.scheduler)) {
				// init part
				schedulerSettings = new EObjectFlatComboSettings(softwareMutualExclusionResource, GrmPackage.eINSTANCE.getMutualExclusionResource_Scheduler());
			...
			if (isAccessible(SrmViewsRepository.General.Properties.scheduler)) {
				generalPart.addFilterToScheduler(new ViewerFilter() {
					 ...
					 */
					public boolean select(Viewer viewer, Object parentElement, Object element) {
						return (element instanceof String && element.equals("")) || (element instanceof ResourceBroker); //$NON-NLS-1$ 
					}
	...
	public EStructuralFeature associatedFeature(Object editorKey) {
		...
		if (editorKey == SrmViewsRepository.General.Properties.scheduler) {
			return GrmPackage.eINSTANCE.getMutualExclusionResource_Scheduler();
		}
	...
	public void updateSemanticModel(final IPropertiesEditionEvent event) {
		...
		if (SrmViewsRepository.General.Properties.name == event.getAffectedEditor()) {
			softwareMutualExclusionResource.setName((java.lang.String)EEFConverterUtil.createFromString(EcorePackage.Literals.ESTRING, (String)event.getNewValue()));
		}
		...
		if (SrmViewsRepository.General.Properties.resMult == event.getAffectedEditor()) {
			softwareMutualExclusionResource.setResMult((EEFConverterUtil.createIntFromString(EcorePackage.Literals.EINT, (String)event.getNewValue())));
		}
		if (SrmViewsRepository.General.Properties.isProtected == event.getAffectedEditor()) {
			softwareMutualExclusionResource.setIsProtected((Boolean)event.getNewValue());
		}
		if (SrmViewsRepository.General.Properties.isActive == event.getAffectedEditor()) {
			softwareMutualExclusionResource.setIsActive((Boolean)event.getNewValue());
		}
		...
		if (SrmViewsRepository.General.Properties.memorySizeFootprint == event.getAffectedEditor()) {
			softwareMutualExclusionResource.setMemorySizeFootprint((EEFConverterUtil.createIntFromString(EcorePackage.Literals.EINT, (String)event.getNewValue())));
		}
		if (SrmViewsRepository.General.Properties.stateElements == event.getAffectedEditor()) {
			if (event.getKind() == PropertiesEditionEvent.SET) {
				softwareMutualExclusionResource.getStateElements().clear();
				softwareMutualExclusionResource.getStateElements().addAll(((EList) event.getNewValue()));
			}
		}
		if (SrmViewsRepository.General.Properties.identifierElements == event.getAffectedEditor()) {
			if (event.getKind() == PropertiesEditionEvent.SET) {
				softwareMutualExclusionResource.getIdentifierElements().clear();
				softwareMutualExclusionResource.getIdentifierElements().addAll(((EList) event.getNewValue()));
			}
		}
		if (SrmViewsRepository.General.Properties.scheduler == event.getAffectedEditor()) {
			if (event.getKind() == PropertiesEditionEvent.SET) {
				schedulerSettings.setToReference((ResourceBroker)event.getNewValue());
			} else if (event.getKind() == PropertiesEditionEvent.ADD) {
				ResourceBroker eObject = GrmFactory.eINSTANCE.createResourceBroker();
	...
	public void updatePart(Notification msg) {
		...
		if (editingPart.isVisible()) {
			...
			if (GrmPackage.eINSTANCE.getMutualExclusionResource_Scheduler().equals(msg.getFeature()) && generalPart != null && isAccessible(SrmViewsRepository.General.Properties.scheduler))
	...
	protected NotificationFilter[] getNotificationFilters() {
		NotificationFilter filter = new EStructuralFeatureNotificationFilter(
			...
			GrmPackage.eINSTANCE.getMutualExclusionResource_Scheduler()		);
