 ...
 * 
 */
public class TimerResourceGeneralPropertiesEditionComponent extends SinglePartPropertiesEditingComponent {
	 ...
	 * 
	 */
	public TimerResourceGeneralPropertiesEditionComponent(PropertiesEditingContext editingContext, EObject timerResource, String editing_mode) {
	...
	public void initPart(Object key, int kind, EObject elt, ResourceSet allResource) {
		...
		if (editingPart != null && key == partKey) {
			...
			if (isAccessible(GrmViewsRepository.General.Properties.name))
				generalPart.setName(EEFConverterUtil.convertToString(EcorePackage.Literals.ESTRING, timerResource.getName()));
			
			if (isAccessible(GrmViewsRepository.General.Properties.ownedResource)) {
				ownedResourceSettings = new ReferencesTableSettings(timerResource, GrmPackage.eINSTANCE.getResource_OwnedResource());
				generalPart.initOwnedResource(ownedResourceSettings);
			}
			if (isAccessible(GrmViewsRepository.General.Properties.resMult)) {
				generalPart.setResMult(EEFConverterUtil.convertToString(EcorePackage.Literals.EINT, timerResource.getResMult()));
			}
			
			if (isAccessible(GrmViewsRepository.General.Properties.isProtected)) {
				generalPart.setIsProtected(timerResource.isIsProtected());
			}
			if (isAccessible(GrmViewsRepository.General.Properties.isActive)) {
				generalPart.setIsActive(timerResource.isIsActive());
			}
			if (isAccessible(GrmViewsRepository.General.Properties.ownedPort)) {
				ownedPortSettings = new ReferencesTableSettings(timerResource, GrmPackage.eINSTANCE.getResource_OwnedPort());
				generalPart.initOwnedPort(ownedPortSettings);
			}
			if (isAccessible(GrmViewsRepository.General.Properties.ownedConnector)) {
				ownedConnectorSettings = new ReferencesTableSettings(timerResource, GrmPackage.eINSTANCE.getResource_OwnedConnector());
				generalPart.initOwnedConnector(ownedConnectorSettings);
			}
			if (isAccessible(GrmViewsRepository.General.Properties.usedBy)) {
				usedBySettings = new ReferencesTableSettings(timerResource, GrmPackage.eINSTANCE.getResource_UsedBy());
				generalPart.initUsedBy(usedBySettings);
			}
			if (isAccessible(GrmViewsRepository.General.Properties.isPeriodic)) {
				generalPart.setIsPeriodic(timerResource.isIsPeriodic());
			}
			if (isAccessible(GrmViewsRepository.General.Properties.start)) {
				// init part
				startSettings = new EObjectFlatComboSettings(timerResource, GrmPackage.eINSTANCE.getTimerResource_Start());
				generalPart.initStart(startSettings);
				// set the button mode
				generalPart.setStartButtonMode(ButtonsModeEnum.BROWSE);
			}
			if (isAccessible(GrmViewsRepository.General.Properties.set)) {
				// init part
				setSettings = new EObjectFlatComboSettings(timerResource, GrmPackage.eINSTANCE.getTimerResource_Set());
				generalPart.initSet(setSettings);
				// set the button mode
				generalPart.setSetButtonMode(ButtonsModeEnum.BROWSE);
			}
			if (isAccessible(GrmViewsRepository.General.Properties.get)) {
				// init part
				getSettings = new EObjectFlatComboSettings(timerResource, GrmPackage.eINSTANCE.getTimerResource_Get());
				generalPart.initGet(getSettings);
				// set the button mode
				generalPart.setGetButtonMode(ButtonsModeEnum.BROWSE);
			}
			if (isAccessible(GrmViewsRepository.General.Properties.reset)) {
				// init part
				resetSettings = new EObjectFlatComboSettings(timerResource, GrmPackage.eINSTANCE.getTimerResource_Reset());
				generalPart.initReset(resetSettings);
				// set the button mode
				generalPart.setResetButtonMode(ButtonsModeEnum.BROWSE);
			}
			if (isAccessible(GrmViewsRepository.General.Properties.pause)) {
				// init part
				pauseSettings = new EObjectFlatComboSettings(timerResource, GrmPackage.eINSTANCE.getTimerResource_Pause());
	...
	public EStructuralFeature associatedFeature(Object editorKey) {
		...
		if (editorKey == GrmViewsRepository.General.Properties.isPeriodic) {
			return GrmPackage.eINSTANCE.getTimerResource_IsPeriodic();
		}
		if (editorKey == GrmViewsRepository.General.Properties.start) {
			return GrmPackage.eINSTANCE.getTimerResource_Start();
		}
		if (editorKey == GrmViewsRepository.General.Properties.set) {
			return GrmPackage.eINSTANCE.getTimerResource_Set();
		}
		if (editorKey == GrmViewsRepository.General.Properties.get) {
			return GrmPackage.eINSTANCE.getTimerResource_Get();
		}
		if (editorKey == GrmViewsRepository.General.Properties.reset) {
			return GrmPackage.eINSTANCE.getTimerResource_Reset();
		}
		if (editorKey == GrmViewsRepository.General.Properties.pause) {
	...
	public void updateSemanticModel(final IPropertiesEditionEvent event) {
		...
		if (GrmViewsRepository.General.Properties.name == event.getAffectedEditor()) {
			timerResource.setName((java.lang.String)EEFConverterUtil.createFromString(EcorePackage.Literals.ESTRING, (String)event.getNewValue()));
		}
		...
		if (GrmViewsRepository.General.Properties.resMult == event.getAffectedEditor()) {
			timerResource.setResMult((EEFConverterUtil.createIntFromString(EcorePackage.Literals.EINT, (String)event.getNewValue())));
		}
		if (GrmViewsRepository.General.Properties.isProtected == event.getAffectedEditor()) {
			timerResource.setIsProtected((Boolean)event.getNewValue());
		}
		if (GrmViewsRepository.General.Properties.isActive == event.getAffectedEditor()) {
			timerResource.setIsActive((Boolean)event.getNewValue());
		}
		...
		if (GrmViewsRepository.General.Properties.isPeriodic == event.getAffectedEditor()) {
			timerResource.setIsPeriodic((Boolean)event.getNewValue());
		}
		if (GrmViewsRepository.General.Properties.start == event.getAffectedEditor()) {
		...
		if (GrmViewsRepository.General.Properties.set == event.getAffectedEditor()) {
			if (event.getKind() == PropertiesEditionEvent.SET) {
		...
		if (GrmViewsRepository.General.Properties.get == event.getAffectedEditor()) {
			if (event.getKind() == PropertiesEditionEvent.SET) {
		...
		if (GrmViewsRepository.General.Properties.reset == event.getAffectedEditor()) {
			if (event.getKind() == PropertiesEditionEvent.SET) {
		...
		if (GrmViewsRepository.General.Properties.pause == event.getAffectedEditor()) {
			if (event.getKind() == PropertiesEditionEvent.SET) {
	...
	public void updatePart(Notification msg) {
		...
		if (editingPart.isVisible()) {
			...
			if (GrmPackage.eINSTANCE.getTimerResource_IsPeriodic().equals(msg.getFeature()) && msg.getNotifier().equals(semanticObject) && generalPart != null && isAccessible(GrmViewsRepository.General.Properties.isPeriodic))
				generalPart.setIsPeriodic((Boolean)msg.getNewValue());
			
			if (GrmPackage.eINSTANCE.getTimerResource_Start().equals(msg.getFeature()) && generalPart != null && isAccessible(GrmViewsRepository.General.Properties.start))
				generalPart.setStart((EObject)msg.getNewValue());
			if (GrmPackage.eINSTANCE.getTimerResource_Set().equals(msg.getFeature()) && generalPart != null && isAccessible(GrmViewsRepository.General.Properties.set))
				generalPart.setSet((EObject)msg.getNewValue());
			if (GrmPackage.eINSTANCE.getTimerResource_Get().equals(msg.getFeature()) && generalPart != null && isAccessible(GrmViewsRepository.General.Properties.get))
				generalPart.setGet((EObject)msg.getNewValue());
			if (GrmPackage.eINSTANCE.getTimerResource_Reset().equals(msg.getFeature()) && generalPart != null && isAccessible(GrmViewsRepository.General.Properties.reset))
				generalPart.setReset((EObject)msg.getNewValue());
			if (GrmPackage.eINSTANCE.getTimerResource_Pause().equals(msg.getFeature()) && generalPart != null && isAccessible(GrmViewsRepository.General.Properties.pause))
