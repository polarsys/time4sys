 ...
 * 
 */
public class CommunicationStepGeneralPropertiesEditionComponent extends SinglePartPropertiesEditingComponent {
	...
	/**
	 * Settings for comRes EObjectFlatComboViewer
	 */
	private EObjectFlatComboSettings comResSettings;
	 ...
	 * 
	 */
	public CommunicationStepGeneralPropertiesEditionComponent(PropertiesEditingContext editingContext, EObject communicationStep, String editing_mode) {
		super(editingContext, communicationStep, editing_mode);
	...
	public void initPart(Object key, int kind, EObject elt, ResourceSet allResource) {
		...
		if (editingPart != null && key == partKey) {
			...
			final CommunicationStep communicationStep = (CommunicationStep)elt;
			final GeneralPropertiesEditionPart generalPart = (GeneralPropertiesEditionPart)editingPart;
			// init values
			if (isAccessible(GqamViewsRepository.General.Properties.name))
				generalPart.setName(EEFConverterUtil.convertToString(EcorePackage.Literals.ESTRING, communicationStep.getName()));
			
			if (isAccessible(GqamViewsRepository.General.Properties.isAtomic)) {
				generalPart.setIsAtomic(communicationStep.isIsAtomic());
			}
			if (isAccessible(GqamViewsRepository.General.Properties.msgSize)) {
				generalPart.setMsgSize(EEFConverterUtil.convertToString(EcorePackage.Literals.EINT, communicationStep.getMsgSize()));
			}
			
			if (isAccessible(GqamViewsRepository.General.Properties.repetitions)) {
				generalPart.setRepetitions(EEFConverterUtil.convertToString(EcorePackage.Literals.EDOUBLE, communicationStep.getRepetitions()));
			}
			
			if (isAccessible(GqamViewsRepository.General.Properties.probability)) {
				generalPart.setProbability(EEFConverterUtil.convertToString(EcorePackage.Literals.EDOUBLE, communicationStep.getProbability()));
			}
			
			if (isAccessible(GqamViewsRepository.General.Properties.priority)) {
				generalPart.setPriority(EEFConverterUtil.convertToString(EcorePackage.Literals.EINT, communicationStep.getPriority()));
			}
			
			if (isAccessible(GqamViewsRepository.General.Properties.serviceCount)) {
				generalPart.setServiceCount(EEFConverterUtil.convertToString(EcorePackage.Literals.EINT, communicationStep.getServiceCount()));
			}
			
			if (isAccessible(GqamViewsRepository.General.Properties.concurRes)) {
				// init part
				concurResSettings = new EObjectFlatComboSettings(communicationStep, GqamPackage.eINSTANCE.getStep_ConcurRes());
				generalPart.initConcurRes(concurResSettings);
				// set the button mode
				generalPart.setConcurResButtonMode(ButtonsModeEnum.BROWSE);
			}
			if (isAccessible(GqamViewsRepository.General.Properties.comRes)) {
				// init part
				comResSettings = new EObjectFlatComboSettings(communicationStep, GqamPackage.eINSTANCE.getCommunicationStep_ComRes());
				generalPart.initComRes(comResSettings);
				// set the button mode
				generalPart.setComResButtonMode(ButtonsModeEnum.BROWSE);
			}
			if (isAccessible(GqamViewsRepository.General.Properties.root)) {
				// init part
				rootSettings = new EObjectFlatComboSettings(communicationStep, GqamPackage.eINSTANCE.getBehaviorScenario_Root());
			...
			if (isAccessible(GqamViewsRepository.General.Properties.parentStep)) {
				// init part
				parentStepSettings = new EObjectFlatComboSettings(communicationStep, GqamPackage.eINSTANCE.getBehaviorScenario_ParentStep());
			...
			if (isAccessible(GqamViewsRepository.General.Properties.scenario)) {
				// init part
				scenarioSettings = new EObjectFlatComboSettings(communicationStep, GqamPackage.eINSTANCE.getStep_Scenario());
			...
			if (isAccessible(GqamViewsRepository.General.Properties.childScenario)) {
				// init part
				childScenarioSettings = new EObjectFlatComboSettings(communicationStep, GqamPackage.eINSTANCE.getStep_ChildScenario());
			...
			if (isAccessible(GqamViewsRepository.General.Properties.inputRel)) {
				// init part
				inputRelSettings = new EObjectFlatComboSettings(communicationStep, GqamPackage.eINSTANCE.getStep_InputRel());
			...
			if (isAccessible(GqamViewsRepository.General.Properties.outputRel)) {
				// init part
				outputRelSettings = new EObjectFlatComboSettings(communicationStep, GqamPackage.eINSTANCE.getStep_OutputRel());
			...
			if (isAccessible(GqamViewsRepository.General.Properties.cause)) {
				causeSettings = new ReferencesTableSettings(communicationStep, GqamPackage.eINSTANCE.getBehaviorScenario_Cause());
				generalPart.initCause(causeSettings);
			}
			if (isAccessible(GqamViewsRepository.General.Properties.steps)) {
				stepsSettings = new ReferencesTableSettings(communicationStep, GqamPackage.eINSTANCE.getBehaviorScenario_Steps());
				generalPart.initSteps(stepsSettings);
			}
			if (isAccessible(GqamViewsRepository.General.Properties.connectors)) {
				connectorsSettings = new ReferencesTableSettings(communicationStep, GqamPackage.eINSTANCE.getBehaviorScenario_Connectors());
			...
			if (isAccessible(GqamViewsRepository.General.Properties.comRes)) {
				generalPart.addFilterToComRes(new ViewerFilter() {
					 ...
					 */
					public boolean select(Viewer viewer, Object parentElement, Object element) {
						return (element instanceof String && element.equals("")) || (element instanceof CommunicationResource); //$NON-NLS-1$ 
					}
					
				});
				// Start of user code for additional businessfilters for comRes
				// End of user code
			}
			if (isAccessible(GqamViewsRepository.General.Properties.root)) {
	...
	public EStructuralFeature associatedFeature(Object editorKey) {
		...
		if (editorKey == GqamViewsRepository.General.Properties.msgSize) {
			return GqamPackage.eINSTANCE.getCommunicationStep_MsgSize();
		}
		...
		if (editorKey == GqamViewsRepository.General.Properties.comRes) {
			return GqamPackage.eINSTANCE.getCommunicationStep_ComRes();
		}
	...
	public void updateSemanticModel(final IPropertiesEditionEvent event) {
		...
		if (GqamViewsRepository.General.Properties.name == event.getAffectedEditor()) {
			communicationStep.setName((java.lang.String)EEFConverterUtil.createFromString(EcorePackage.Literals.ESTRING, (String)event.getNewValue()));
		}
		...
		if (GqamViewsRepository.General.Properties.probability == event.getAffectedEditor()) {
			communicationStep.setProbability((EEFConverterUtil.createDoubleFromString(EcorePackage.Literals.EDOUBLE, (String)event.getNewValue())));
		}
		if (GqamViewsRepository.General.Properties.priority == event.getAffectedEditor()) {
			communicationStep.setPriority((EEFConverterUtil.createIntFromString(EcorePackage.Literals.EINT, (String)event.getNewValue())));
		}
		if (GqamViewsRepository.General.Properties.serviceCount == event.getAffectedEditor()) {
			communicationStep.setServiceCount((EEFConverterUtil.createIntFromString(EcorePackage.Literals.EINT, (String)event.getNewValue())));
		}
		...
		if (GqamViewsRepository.General.Properties.comRes == event.getAffectedEditor()) {
			if (event.getKind() == PropertiesEditionEvent.SET) {
				comResSettings.setToReference((CommunicationResource)event.getNewValue());
			} else if (event.getKind() == PropertiesEditionEvent.ADD) {
				EReferencePropertiesEditionContext context = new EReferencePropertiesEditionContext(editingContext, this, comResSettings, editingContext.getAdapterFactory());
	...
	public void updatePart(Notification msg) {
		...
		if (editingPart.isVisible()) {
			...
			if (GqamPackage.eINSTANCE.getCommunicationStep_MsgSize().equals(msg.getFeature()) && msg.getNotifier().equals(semanticObject) && generalPart != null && isAccessible(GqamViewsRepository.General.Properties.msgSize)) {
				if (msg.getNewValue() != null) {
					generalPart.setMsgSize(EcoreUtil.convertToString(EcorePackage.Literals.EINT, msg.getNewValue()));
				} else {
					generalPart.setMsgSize("");
				}
			}
			...
			if (GqamPackage.eINSTANCE.getCommunicationStep_ComRes().equals(msg.getFeature()) && generalPart != null && isAccessible(GqamViewsRepository.General.Properties.comRes))
				generalPart.setComRes((EObject)msg.getNewValue());
	...
	protected NotificationFilter[] getNotificationFilters() {
		NotificationFilter filter = new EStructuralFeatureNotificationFilter(
			...
			GqamPackage.eINSTANCE.getCommunicationStep_MsgSize(),
			...
			GqamPackage.eINSTANCE.getCommunicationStep_ComRes(),
		...
		if (event.getNewValue() != null) {
			try {
				...
				if (GqamViewsRepository.General.Properties.msgSize == event.getAffectedEditor()) {
					Object newValue = event.getNewValue();
					if (newValue instanceof String) {
						newValue = EEFConverterUtil.createFromString(GqamPackage.eINSTANCE.getCommunicationStep_MsgSize().getEAttributeType(), (String)newValue);
					}
					ret = Diagnostician.INSTANCE.validate(GqamPackage.eINSTANCE.getCommunicationStep_MsgSize().getEAttributeType(), newValue);
				}
				if (GqamViewsRepository.General.Properties.repetitions == event.getAffectedEditor()) {
					Object newValue = event.getNewValue();
					if (newValue instanceof String) {
