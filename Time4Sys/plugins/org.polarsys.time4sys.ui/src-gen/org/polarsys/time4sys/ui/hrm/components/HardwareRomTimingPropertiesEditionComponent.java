 ...
 * 
 */
public class HardwareRomTimingPropertiesEditionComponent extends SinglePartPropertiesEditingComponent {
	 ...
	 * 
	 */
	public HardwareRomTimingPropertiesEditionComponent(PropertiesEditingContext editingContext, EObject hardwareRom, String editing_mode) {
	...
	public void initPart(Object key, int kind, EObject elt, ResourceSet allResource) {
		...
		if (editingPart != null && key == partKey) {
			...
			if (isAccessible(HrmViewsRepository.Timing.TimingProperties.clock)) {
				// init part
				clockSettings = new EObjectFlatComboSettings(hardwareRom, HrmPackage.eINSTANCE.getHardwareResource_Clock());
			...
			if (isAccessible(HrmViewsRepository.Timing.TimingProperties.timings))
				timingPart.setTimings(hardwareRom.getTimings());
	...
	public void updateSemanticModel(final IPropertiesEditionEvent event) {
		...
		if (HrmViewsRepository.Timing.TimingProperties.timings == event.getAffectedEditor()) {
			if (event.getKind() == PropertiesEditionEvent.SET) {
				hardwareRom.getTimings().clear();
				hardwareRom.getTimings().addAll(((EList) event.getNewValue()));
			}
		}
	}
	...
	public boolean mustBeComposed(Object key, int kind) {
		return key == HrmViewsRepository.Timing.TimingProperties.clock || key == HrmViewsRepository.Timing.TimingProperties.timings || key == HrmViewsRepository.Timing.TimingProperties.class;
	}
