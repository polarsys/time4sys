 ...
 * 
 */
public class AcquireStepTimingPropertiesEditionComponent extends SinglePartPropertiesEditingComponent {
	 ...
	 * 
	 */
	public AcquireStepTimingPropertiesEditionComponent(PropertiesEditingContext editingContext, EObject acquireStep, String editing_mode) {
	...
	public void initPart(Object key, int kind, EObject elt, ResourceSet allResource) {
		...
		if (editingPart != null && key == partKey) {
			...
			if (isAccessible(GqamViewsRepository.Timing.TimingProperties.respTime))
				timingPart.setRespTime(EEFConverterUtil.convertToString(GqamPackage.Literals.NFP_DURATION, acquireStep.getRespTime()));
			
			if (isAccessible(GqamViewsRepository.Timing.TimingProperties.bestCET))
				timingPart.setBestCET(EEFConverterUtil.convertToString(GqamPackage.Literals.NFP_DURATION, acquireStep.getBestCET()));
			
			if (isAccessible(GqamViewsRepository.Timing.TimingProperties.worstCET))
				timingPart.setWorstCET(EEFConverterUtil.convertToString(GqamPackage.Literals.NFP_DURATION, acquireStep.getWorstCET()));
			
			if (isAccessible(GqamViewsRepository.Timing.TimingProperties.blockingTime))
				timingPart.setBlockingTime(EEFConverterUtil.convertToString(GqamPackage.Literals.NFP_DURATION, acquireStep.getBlockingTime()));
	...
	public void updateSemanticModel(final IPropertiesEditionEvent event) {
		...
		if (GqamViewsRepository.Timing.TimingProperties.respTime == event.getAffectedEditor()) {
			acquireStep.setRespTime((org.polarsys.time4sys.marte.nfp.Duration)EEFConverterUtil.createFromString(GqamPackage.Literals.NFP_DURATION, (String)event.getNewValue()));
		}
		if (GqamViewsRepository.Timing.TimingProperties.bestCET == event.getAffectedEditor()) {
			acquireStep.setBestCET((org.polarsys.time4sys.marte.nfp.Duration)EEFConverterUtil.createFromString(GqamPackage.Literals.NFP_DURATION, (String)event.getNewValue()));
		}
		if (GqamViewsRepository.Timing.TimingProperties.worstCET == event.getAffectedEditor()) {
			acquireStep.setWorstCET((org.polarsys.time4sys.marte.nfp.Duration)EEFConverterUtil.createFromString(GqamPackage.Literals.NFP_DURATION, (String)event.getNewValue()));
		}
		if (GqamViewsRepository.Timing.TimingProperties.blockingTime == event.getAffectedEditor()) {
			acquireStep.setBlockingTime((org.polarsys.time4sys.marte.nfp.Duration)EEFConverterUtil.createFromString(GqamPackage.Literals.NFP_DURATION, (String)event.getNewValue()));
		}
	}
