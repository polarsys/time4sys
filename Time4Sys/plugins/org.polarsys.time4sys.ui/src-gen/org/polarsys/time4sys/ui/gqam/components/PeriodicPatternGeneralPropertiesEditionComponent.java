 ...
 * 
 */
public class PeriodicPatternGeneralPropertiesEditionComponent extends SinglePartPropertiesEditingComponent {
	 ...
	 * 
	 */
	public PeriodicPatternGeneralPropertiesEditionComponent(PropertiesEditingContext editingContext, EObject periodicPattern, String editing_mode) {
	...
	public void initPart(Object key, int kind, EObject elt, ResourceSet allResource) {
		...
		if (editingPart != null && key == partKey) {
			...
			// init values
			if (isAccessible(GqamViewsRepository.General.Properties.occurences)) {
				generalPart.setOccurences(EEFConverterUtil.convertToString(EcorePackage.Literals.EINT, periodicPattern.getOccurences()));
			}
			
			if (isAccessible(GqamViewsRepository.General.Properties.parent_)) {
				// init part
				parent_Settings = new EObjectFlatComboSettings(periodicPattern, GqamPackage.eINSTANCE.getArrivalPattern_Parent());
				generalPart.initParent_(parent_Settings);
				// set the button mode
				generalPart.setParent_ButtonMode(ButtonsModeEnum.BROWSE);
			}
			if (isAccessible(GqamViewsRepository.General.Properties.reference)) {
				// init part
				referenceSettings = new EObjectFlatComboSettings(periodicPattern, GqamPackage.eINSTANCE.getArrivalPattern_Reference());
				generalPart.initReference(referenceSettings);
				// set the button mode
				generalPart.setReferenceButtonMode(ButtonsModeEnum.BROWSE);
			}
			// init filters
			...
			if (isAccessible(GqamViewsRepository.General.Properties.reference)) {
				generalPart.addFilterToReference(new ViewerFilter() {
	...
	public EStructuralFeature associatedFeature(Object editorKey) {
		if (editorKey == GqamViewsRepository.General.Properties.occurences) {
			return GqamPackage.eINSTANCE.getPeriodicPattern_Occurences();
		}
		if (editorKey == GqamViewsRepository.General.Properties.parent_) {
			return GqamPackage.eINSTANCE.getArrivalPattern_Parent();
		}
		if (editorKey == GqamViewsRepository.General.Properties.reference) {
			return GqamPackage.eINSTANCE.getArrivalPattern_Reference();
		}
	...
	public void updateSemanticModel(final IPropertiesEditionEvent event) {
		...
		if (GqamViewsRepository.General.Properties.occurences == event.getAffectedEditor()) {
			periodicPattern.setOccurences((EEFConverterUtil.createIntFromString(EcorePackage.Literals.EINT, (String)event.getNewValue())));
		}
		...
		if (GqamViewsRepository.General.Properties.reference == event.getAffectedEditor()) {
			if (event.getKind() == PropertiesEditionEvent.SET) {
				referenceSettings.setToReference((Reference)event.getNewValue());
			} else if (event.getKind() == PropertiesEditionEvent.ADD) {
				Reference eObject = GqamFactory.eINSTANCE.createReference();
				EObjectPropertiesEditionContext context = new EObjectPropertiesEditionContext(editingContext, this, eObject, editingContext.getAdapterFactory());
				PropertiesEditingProvider provider = (PropertiesEditingProvider)editingContext.getAdapterFactory().adapt(eObject, PropertiesEditingProvider.class);
				if (provider != null) {
					PropertiesEditingPolicy policy = provider.getPolicy(context);
	...
	protected NotificationFilter[] getNotificationFilters() {
		NotificationFilter filter = new EStructuralFeatureNotificationFilter(
			GqamPackage.eINSTANCE.getPeriodicPattern_Occurences(),
			GqamPackage.eINSTANCE.getArrivalPattern_Parent(),
			GqamPackage.eINSTANCE.getArrivalPattern_Reference());
	...
	public boolean mustBeComposed(Object key, int kind) {
		return key == GqamViewsRepository.General.Properties.occurences || key == GqamViewsRepository.General.Properties.parent_ || key == GqamViewsRepository.General.Properties.reference || key == GqamViewsRepository.General.Properties.class;
	}
