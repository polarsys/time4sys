 ...
 * 
 */
public class HardwareRamGeneralPropertiesEditionComponent extends SinglePartPropertiesEditingComponent {
	 ...
	 * 
	 */
	public HardwareRamGeneralPropertiesEditionComponent(PropertiesEditingContext editingContext, EObject hardwareRam, String editing_mode) {
	...
	public void initPart(Object key, int kind, EObject elt, ResourceSet allResource) {
		...
		if (editingPart != null && key == partKey) {
			...
			if (isAccessible(HrmViewsRepository.General.Properties.name))
				generalPart.setName(EEFConverterUtil.convertToString(EcorePackage.Literals.ESTRING, hardwareRam.getName()));
			
			if (isAccessible(HrmViewsRepository.General.Properties.ownedResource)) {
				ownedResourceSettings = new ReferencesTableSettings(hardwareRam, GrmPackage.eINSTANCE.getResource_OwnedResource());
				generalPart.initOwnedResource(ownedResourceSettings);
			}
			if (isAccessible(HrmViewsRepository.General.Properties.resMult)) {
				generalPart.setResMult(EEFConverterUtil.convertToString(EcorePackage.Literals.EINT, hardwareRam.getResMult()));
			}
			
			if (isAccessible(HrmViewsRepository.General.Properties.isProtected)) {
				generalPart.setIsProtected(hardwareRam.isIsProtected());
			}
			if (isAccessible(HrmViewsRepository.General.Properties.isActive)) {
				generalPart.setIsActive(hardwareRam.isIsActive());
			}
			if (isAccessible(HrmViewsRepository.General.Properties.ownedConnector)) {
				ownedConnectorSettings = new ReferencesTableSettings(hardwareRam, GrmPackage.eINSTANCE.getResource_OwnedConnector());
				generalPart.initOwnedConnector(ownedConnectorSettings);
			}
			if (isAccessible(HrmViewsRepository.General.Properties.usedBy)) {
				usedBySettings = new ReferencesTableSettings(hardwareRam, GrmPackage.eINSTANCE.getResource_UsedBy());
				generalPart.initUsedBy(usedBySettings);
			}
			if (isAccessible(HrmViewsRepository.General.Properties.memorySize)) {
				generalPart.setMemorySize(EEFConverterUtil.convertToString(EcorePackage.Literals.EINT, hardwareRam.getMemorySize()));
			}
			
			if (isAccessible(HrmViewsRepository.General.Properties.addressSize)) {
				generalPart.setAddressSize(EEFConverterUtil.convertToString(EcorePackage.Literals.EINT, hardwareRam.getAddressSize()));
			}
			
			if (isAccessible(HrmViewsRepository.General.Properties.throughput)) {
				generalPart.setThroughput(EEFConverterUtil.convertToString(EcorePackage.Literals.EINT, hardwareRam.getThroughput()));
			}
			
			if (isAccessible(HrmViewsRepository.General.Properties.isSynchronous)) {
				generalPart.setIsSynchronous(hardwareRam.isIsSynchronous());
			}
			if (isAccessible(HrmViewsRepository.General.Properties.isStatic)) {
				generalPart.setIsStatic(hardwareRam.isIsStatic());
			}
			if (isAccessible(HrmViewsRepository.General.Properties.isNonVolatile)) {
				generalPart.setIsNonVolatile(hardwareRam.isIsNonVolatile());
			}
			// init filters
	...
	public EStructuralFeature associatedFeature(Object editorKey) {
		...
		if (editorKey == HrmViewsRepository.General.Properties.isSynchronous) {
			return HrmPackage.eINSTANCE.getHardwareRam_IsSynchronous();
		}
		if (editorKey == HrmViewsRepository.General.Properties.isStatic) {
			return HrmPackage.eINSTANCE.getHardwareRam_IsStatic();
		}
		if (editorKey == HrmViewsRepository.General.Properties.isNonVolatile) {
			return HrmPackage.eINSTANCE.getHardwareRam_IsNonVolatile();
		}
	...
	public void updateSemanticModel(final IPropertiesEditionEvent event) {
		...
		if (HrmViewsRepository.General.Properties.name == event.getAffectedEditor()) {
			hardwareRam.setName((java.lang.String)EEFConverterUtil.createFromString(EcorePackage.Literals.ESTRING, (String)event.getNewValue()));
		}
		...
		if (HrmViewsRepository.General.Properties.resMult == event.getAffectedEditor()) {
			hardwareRam.setResMult((EEFConverterUtil.createIntFromString(EcorePackage.Literals.EINT, (String)event.getNewValue())));
		}
		if (HrmViewsRepository.General.Properties.isProtected == event.getAffectedEditor()) {
			hardwareRam.setIsProtected((Boolean)event.getNewValue());
		}
		if (HrmViewsRepository.General.Properties.isActive == event.getAffectedEditor()) {
			hardwareRam.setIsActive((Boolean)event.getNewValue());
		}
		...
		if (HrmViewsRepository.General.Properties.memorySize == event.getAffectedEditor()) {
			hardwareRam.setMemorySize((EEFConverterUtil.createIntFromString(EcorePackage.Literals.EINT, (String)event.getNewValue())));
		}
		if (HrmViewsRepository.General.Properties.addressSize == event.getAffectedEditor()) {
			hardwareRam.setAddressSize((EEFConverterUtil.createIntFromString(EcorePackage.Literals.EINT, (String)event.getNewValue())));
		}
		if (HrmViewsRepository.General.Properties.throughput == event.getAffectedEditor()) {
			hardwareRam.setThroughput((EEFConverterUtil.createIntFromString(EcorePackage.Literals.EINT, (String)event.getNewValue())));
		}
		if (HrmViewsRepository.General.Properties.isSynchronous == event.getAffectedEditor()) {
			hardwareRam.setIsSynchronous((Boolean)event.getNewValue());
		}
		if (HrmViewsRepository.General.Properties.isStatic == event.getAffectedEditor()) {
			hardwareRam.setIsStatic((Boolean)event.getNewValue());
		}
		if (HrmViewsRepository.General.Properties.isNonVolatile == event.getAffectedEditor()) {
			hardwareRam.setIsNonVolatile((Boolean)event.getNewValue());
		}
	}
	...
	protected NotificationFilter[] getNotificationFilters() {
		NotificationFilter filter = new EStructuralFeatureNotificationFilter(
			...
			HrmPackage.eINSTANCE.getHardwareRam_IsSynchronous(),
			HrmPackage.eINSTANCE.getHardwareRam_IsStatic(),
			HrmPackage.eINSTANCE.getHardwareRam_IsNonVolatile()		);
	...
	public boolean mustBeComposed(Object key, int kind) {
		return key == HrmViewsRepository.General.Properties.name || key == HrmViewsRepository.General.Properties.ownedResource || key == HrmViewsRepository.General.Properties.resMult || key == HrmViewsRepository.General.Properties.isProtected || key == HrmViewsRepository.General.Properties.isActive || key == HrmViewsRepository.General.Properties.ownedConnector || key == HrmViewsRepository.General.Properties.usedBy || key == HrmViewsRepository.General.Properties.memorySize || key == HrmViewsRepository.General.Properties.addressSize || key == HrmViewsRepository.General.Properties.throughput || key == HrmViewsRepository.General.Properties.isSynchronous || key == HrmViewsRepository.General.Properties.isStatic || key == HrmViewsRepository.General.Properties.isNonVolatile || key == HrmViewsRepository.General.Properties.class;
	}
		...
		if (event.getNewValue() != null) {
			try {
				...
				if (HrmViewsRepository.General.Properties.isSynchronous == event.getAffectedEditor()) {
					Object newValue = event.getNewValue();
					if (newValue instanceof String) {
						newValue = EEFConverterUtil.createFromString(HrmPackage.eINSTANCE.getHardwareRam_IsSynchronous().getEAttributeType(), (String)newValue);
					}
					ret = Diagnostician.INSTANCE.validate(HrmPackage.eINSTANCE.getHardwareRam_IsSynchronous().getEAttributeType(), newValue);
				}
				if (HrmViewsRepository.General.Properties.isStatic == event.getAffectedEditor()) {
					Object newValue = event.getNewValue();
					if (newValue instanceof String) {
						newValue = EEFConverterUtil.createFromString(HrmPackage.eINSTANCE.getHardwareRam_IsStatic().getEAttributeType(), (String)newValue);
					}
					ret = Diagnostician.INSTANCE.validate(HrmPackage.eINSTANCE.getHardwareRam_IsStatic().getEAttributeType(), newValue);
				}
				if (HrmViewsRepository.General.Properties.isNonVolatile == event.getAffectedEditor()) {
					Object newValue = event.getNewValue();
					if (newValue instanceof String) {
						newValue = EEFConverterUtil.createFromString(HrmPackage.eINSTANCE.getHardwareRam_IsNonVolatile().getEAttributeType(), (String)newValue);
					}
					ret = Diagnostician.INSTANCE.validate(HrmPackage.eINSTANCE.getHardwareRam_IsNonVolatile().getEAttributeType(), newValue);
				}
