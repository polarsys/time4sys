 ...
 * 
 */
public class DeviceBrokerTimingPropertiesEditionComponent extends SinglePartPropertiesEditingComponent {
	 ...
	 * 
	 */
	public DeviceBrokerTimingPropertiesEditionComponent(PropertiesEditingContext editingContext, EObject deviceBroker, String editing_mode) {
	...
	public void initPart(Object key, int kind, EObject elt, ResourceSet allResource) {
		...
		if (editingPart != null && key == partKey) {
			...
			if (isAccessible(SrmViewsRepository.Timing.Properties.accCtrlPolicy)) {
				accCtrlPolicySettings = new ReferencesTableSettings(deviceBroker, GrmPackage.eINSTANCE.getResourceBroker_AccCtrlPolicy());
				timingPart.initAccCtrlPolicy(accCtrlPolicySettings);
			}
			if (isAccessible(SrmViewsRepository.Timing.Properties.resCtrlPolicy)) {
				resCtrlPolicySettings = new ReferencesTableSettings(deviceBroker, GrmPackage.eINSTANCE.getResourceManager_ResCtrlPolicy());
				timingPart.initResCtrlPolicy(resCtrlPolicySettings);
			}
			if (isAccessible(SrmViewsRepository.Timing.Properties.accessPolicy)) {
				timingPart.initAccessPolicy(EEFUtils.choiceOfValues(deviceBroker, SrmPackage.eINSTANCE.getDeviceBroker_AccessPolicy()), deviceBroker.getAccessPolicy());
			}
			// init filters
			if (isAccessible(SrmViewsRepository.Timing.Properties.accCtrlPolicy)) {
	...
	public EStructuralFeature associatedFeature(Object editorKey) {
		...
		if (editorKey == SrmViewsRepository.Timing.Properties.accessPolicy) {
			return SrmPackage.eINSTANCE.getDeviceBroker_AccessPolicy();
		}
	...
	public void updateSemanticModel(final IPropertiesEditionEvent event) {
		...
		if (SrmViewsRepository.Timing.Properties.accessPolicy == event.getAffectedEditor()) {
			deviceBroker.setAccessPolicy((AccessPolicyKind)event.getNewValue());
		}
	}
	...
	public void updatePart(Notification msg) {
		...
		if (editingPart.isVisible()) {
			...
			if (SrmPackage.eINSTANCE.getDeviceBroker_AccessPolicy().equals(msg.getFeature()) && msg.getNotifier().equals(semanticObject) && isAccessible(SrmViewsRepository.Timing.Properties.accessPolicy))
	...
	protected NotificationFilter[] getNotificationFilters() {
		NotificationFilter filter = new EStructuralFeatureNotificationFilter(
			...
			SrmPackage.eINSTANCE.getDeviceBroker_AccessPolicy()		);
