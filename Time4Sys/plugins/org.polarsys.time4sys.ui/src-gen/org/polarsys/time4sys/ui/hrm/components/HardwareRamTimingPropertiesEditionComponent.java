 ...
 * 
 */
public class HardwareRamTimingPropertiesEditionComponent extends SinglePartPropertiesEditingComponent {
	 ...
	 * 
	 */
	public HardwareRamTimingPropertiesEditionComponent(PropertiesEditingContext editingContext, EObject hardwareRam, String editing_mode) {
	...
	public void initPart(Object key, int kind, EObject elt, ResourceSet allResource) {
		...
		if (editingPart != null && key == partKey) {
			...
			if (isAccessible(HrmViewsRepository.Timing.TimingProperties.clock)) {
				// init part
				clockSettings = new EObjectFlatComboSettings(hardwareRam, HrmPackage.eINSTANCE.getHardwareResource_Clock());
			...
			if (isAccessible(HrmViewsRepository.Timing.TimingProperties.timings))
				timingPart.setTimings(hardwareRam.getTimings());
			
			if (isAccessible(HrmViewsRepository.Timing.TimingProperties.replPolicy)) {
				timingPart.initReplPolicy(EEFUtils.choiceOfValues(hardwareRam, HrmPackage.eINSTANCE.getHardwareProcessingMemory_ReplPolicy()), hardwareRam.getReplPolicy());
			}
			if (isAccessible(HrmViewsRepository.Timing.TimingProperties.writePolicy)) {
				timingPart.initWritePolicy(EEFUtils.choiceOfValues(hardwareRam, HrmPackage.eINSTANCE.getHardwareProcessingMemory_WritePolicy()), hardwareRam.getWritePolicy());
			}
			// init filters
			if (isAccessible(HrmViewsRepository.Timing.TimingProperties.clock)) {
	...
	public void updateSemanticModel(final IPropertiesEditionEvent event) {
		...
		if (HrmViewsRepository.Timing.TimingProperties.timings == event.getAffectedEditor()) {
			if (event.getKind() == PropertiesEditionEvent.SET) {
				hardwareRam.getTimings().clear();
				hardwareRam.getTimings().addAll(((EList) event.getNewValue()));
			}
		}
		if (HrmViewsRepository.Timing.TimingProperties.replPolicy == event.getAffectedEditor()) {
			hardwareRam.setReplPolicy((ReplPolicy)event.getNewValue());
		}
		if (HrmViewsRepository.Timing.TimingProperties.writePolicy == event.getAffectedEditor()) {
			hardwareRam.setWritePolicy((WritePolicy)event.getNewValue());
		}
	}
