 ...
 * 
 */
public class AlarmTimingPropertiesEditionComponent extends SinglePartPropertiesEditingComponent {
	...
	/**
	 * Settings for timers ReferencesTable
	 */
	private ReferencesTableSettings timersSettings;
	 ...
	 * 
	 */
	public AlarmTimingPropertiesEditionComponent(PropertiesEditingContext editingContext, EObject alarm, String editing_mode) {
	...
	public void initPart(Object key, int kind, EObject elt, ResourceSet allResource) {
		...
		if (editingPart != null && key == partKey) {
			...
			if (isAccessible(SrmViewsRepository.Timing.Properties.resCtrlPolicy)) {
				resCtrlPolicySettings = new ReferencesTableSettings(alarm, GrmPackage.eINSTANCE.getResourceManager_ResCtrlPolicy());
				timingPart.initResCtrlPolicy(resCtrlPolicySettings);
			}
			if (isAccessible(SrmViewsRepository.Timing.Properties.priorityElements))
				timingPart.setPriorityElements(alarm.getPriorityElements());
			
			if (isAccessible(SrmViewsRepository.Timing.Properties.mutualExclusionResources)) {
				mutualExclusionResourcesSettings = new ReferencesTableSettings(alarm, SrmPackage.eINSTANCE.getSoftwareConcurrentResource_MutualExclusionResources());
			...
			if (isAccessible(SrmViewsRepository.Timing.Properties.arrivalPattern))
				timingPart.setArrivalPattern(EEFConverterUtil.convertToString(EcorePackage.Literals.ESTRING, alarm.getArrivalPattern()));
			
			if (isAccessible(SrmViewsRepository.Timing.Properties.timers)) {
				timersSettings = new ReferencesTableSettings(alarm, SrmPackage.eINSTANCE.getAlarm_Timers());
				timingPart.initTimers(timersSettings);
			}
			// init filters
			if (isAccessible(SrmViewsRepository.Timing.Properties.resCtrlPolicy)) {
			...
			if (isAccessible(SrmViewsRepository.Timing.Properties.timers)) {
				timingPart.addFilterToTimers(new EObjectFilter(SrmPackage.Literals.SOFTWARE_TIMER_RESOURCE));
				// Start of user code for additional businessfilters for timers
				// End of user code
			}
	...
	public EStructuralFeature associatedFeature(Object editorKey) {
		...
		if (editorKey == SrmViewsRepository.Timing.Properties.timers) {
			return SrmPackage.eINSTANCE.getAlarm_Timers();
		}
	...
	public void updateSemanticModel(final IPropertiesEditionEvent event) {
		...
		if (SrmViewsRepository.Timing.Properties.priorityElements == event.getAffectedEditor()) {
			if (event.getKind() == PropertiesEditionEvent.SET) {
				alarm.getPriorityElements().clear();
				alarm.getPriorityElements().addAll(((EList) event.getNewValue()));
			}
		}
		...
		if (SrmViewsRepository.Timing.Properties.arrivalPattern == event.getAffectedEditor()) {
			alarm.setArrivalPattern((java.lang.String)EEFConverterUtil.createFromString(EcorePackage.Literals.ESTRING, (String)event.getNewValue()));
		}
		if (SrmViewsRepository.Timing.Properties.timers == event.getAffectedEditor()) {
			if (event.getKind() == PropertiesEditionEvent.ADD) {
				if (event.getNewValue() instanceof SoftwareTimerResource) {
					timersSettings.addToReference((EObject) event.getNewValue());
				}
			} else if (event.getKind() == PropertiesEditionEvent.REMOVE) {
				timersSettings.removeFromReference((EObject) event.getNewValue());
			} else if (event.getKind() == PropertiesEditionEvent.MOVE) {
				timersSettings.move(event.getNewIndex(), (SoftwareTimerResource) event.getNewValue());
			}
		}
	}
	...
	protected NotificationFilter[] getNotificationFilters() {
		NotificationFilter filter = new EStructuralFeatureNotificationFilter(
			...
			SrmPackage.eINSTANCE.getAlarm_Timers()		);
	...
	public boolean mustBeComposed(Object key, int kind) {
		return key == SrmViewsRepository.Timing.Properties.resCtrlPolicy || key == SrmViewsRepository.Timing.Properties.priorityElements || key == SrmViewsRepository.Timing.Properties.mutualExclusionResources || key == SrmViewsRepository.Timing.Properties.arrivalPattern || key == SrmViewsRepository.Timing.Properties.timers || key == SrmViewsRepository.Timing.Properties.class;
	}
