 ...
 * 
 */
public class RequestedServiceTimingPropertiesEditionComponent extends SinglePartPropertiesEditingComponent {
	 ...
	 * 
	 */
	public RequestedServiceTimingPropertiesEditionComponent(PropertiesEditingContext editingContext, EObject requestedService, String editing_mode) {
	...
	public void initPart(Object key, int kind, EObject elt, ResourceSet allResource) {
		...
		if (editingPart != null && key == partKey) {
			...
			if (isAccessible(GqamViewsRepository.Timing.TimingProperties.respTime))
				timingPart.setRespTime(EEFConverterUtil.convertToString(GqamPackage.Literals.NFP_DURATION, requestedService.getRespTime()));
			
			if (isAccessible(GqamViewsRepository.Timing.TimingProperties.bestCET))
				timingPart.setBestCET(EEFConverterUtil.convertToString(GqamPackage.Literals.NFP_DURATION, requestedService.getBestCET()));
			
			if (isAccessible(GqamViewsRepository.Timing.TimingProperties.worstCET))
				timingPart.setWorstCET(EEFConverterUtil.convertToString(GqamPackage.Literals.NFP_DURATION, requestedService.getWorstCET()));
			
			if (isAccessible(GqamViewsRepository.Timing.TimingProperties.blockingTime))
				timingPart.setBlockingTime(EEFConverterUtil.convertToString(GqamPackage.Literals.NFP_DURATION, requestedService.getBlockingTime()));
	...
	public void updateSemanticModel(final IPropertiesEditionEvent event) {
		...
		if (GqamViewsRepository.Timing.TimingProperties.respTime == event.getAffectedEditor()) {
			requestedService.setRespTime((org.polarsys.time4sys.marte.nfp.Duration)EEFConverterUtil.createFromString(GqamPackage.Literals.NFP_DURATION, (String)event.getNewValue()));
		}
		if (GqamViewsRepository.Timing.TimingProperties.bestCET == event.getAffectedEditor()) {
			requestedService.setBestCET((org.polarsys.time4sys.marte.nfp.Duration)EEFConverterUtil.createFromString(GqamPackage.Literals.NFP_DURATION, (String)event.getNewValue()));
		}
		if (GqamViewsRepository.Timing.TimingProperties.worstCET == event.getAffectedEditor()) {
			requestedService.setWorstCET((org.polarsys.time4sys.marte.nfp.Duration)EEFConverterUtil.createFromString(GqamPackage.Literals.NFP_DURATION, (String)event.getNewValue()));
		}
		if (GqamViewsRepository.Timing.TimingProperties.blockingTime == event.getAffectedEditor()) {
			requestedService.setBlockingTime((org.polarsys.time4sys.marte.nfp.Duration)EEFConverterUtil.createFromString(GqamPackage.Literals.NFP_DURATION, (String)event.getNewValue()));
		}
	}
