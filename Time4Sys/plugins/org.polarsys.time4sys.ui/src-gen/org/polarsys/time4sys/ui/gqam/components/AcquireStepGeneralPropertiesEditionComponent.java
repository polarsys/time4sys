 ...
 * 
 */
public class AcquireStepGeneralPropertiesEditionComponent extends SinglePartPropertiesEditingComponent {
	...
	/**
	 * Settings for acqRes EObjectFlatComboViewer
	 */
	private EObjectFlatComboSettings acqResSettings;
	 ...
	 * 
	 */
	public AcquireStepGeneralPropertiesEditionComponent(PropertiesEditingContext editingContext, EObject acquireStep, String editing_mode) {
		super(editingContext, acquireStep, editing_mode);
	...
	public void initPart(Object key, int kind, EObject elt, ResourceSet allResource) {
		...
		if (editingPart != null && key == partKey) {
			...
			final AcquireStep acquireStep = (AcquireStep)elt;
			final GeneralPropertiesEditionPart generalPart = (GeneralPropertiesEditionPart)editingPart;
			// init values
			if (isAccessible(GqamViewsRepository.General.Properties.name))
				generalPart.setName(EEFConverterUtil.convertToString(EcorePackage.Literals.ESTRING, acquireStep.getName()));
			
			if (isAccessible(GqamViewsRepository.General.Properties.isAtomic)) {
				generalPart.setIsAtomic(acquireStep.isIsAtomic());
			}
			if (isAccessible(GqamViewsRepository.General.Properties.repetitions)) {
				generalPart.setRepetitions(EEFConverterUtil.convertToString(EcorePackage.Literals.EDOUBLE, acquireStep.getRepetitions()));
			}
			
			if (isAccessible(GqamViewsRepository.General.Properties.probability)) {
				generalPart.setProbability(EEFConverterUtil.convertToString(EcorePackage.Literals.EDOUBLE, acquireStep.getProbability()));
			}
			
			if (isAccessible(GqamViewsRepository.General.Properties.priority)) {
				generalPart.setPriority(EEFConverterUtil.convertToString(EcorePackage.Literals.EINT, acquireStep.getPriority()));
			}
			
			if (isAccessible(GqamViewsRepository.General.Properties.serviceCount)) {
				generalPart.setServiceCount(EEFConverterUtil.convertToString(EcorePackage.Literals.EINT, acquireStep.getServiceCount()));
			}
			
			if (isAccessible(GqamViewsRepository.General.Properties.resUnits)) {
				generalPart.setResUnits(EEFConverterUtil.convertToString(EcorePackage.Literals.EINT, acquireStep.getResUnits()));
			}
			
			if (isAccessible(GqamViewsRepository.General.Properties.acqRes)) {
				// init part
				acqResSettings = new EObjectFlatComboSettings(acquireStep, GqamPackage.eINSTANCE.getAcquireStep_AcqRes());
				generalPart.initAcqRes(acqResSettings);
				// set the button mode
				generalPart.setAcqResButtonMode(ButtonsModeEnum.BROWSE);
			}
			if (isAccessible(GqamViewsRepository.General.Properties.concurRes)) {
				// init part
				concurResSettings = new EObjectFlatComboSettings(acquireStep, GqamPackage.eINSTANCE.getStep_ConcurRes());
			...
			if (isAccessible(GqamViewsRepository.General.Properties.root)) {
				// init part
				rootSettings = new EObjectFlatComboSettings(acquireStep, GqamPackage.eINSTANCE.getBehaviorScenario_Root());
			...
			if (isAccessible(GqamViewsRepository.General.Properties.parentStep)) {
				// init part
				parentStepSettings = new EObjectFlatComboSettings(acquireStep, GqamPackage.eINSTANCE.getBehaviorScenario_ParentStep());
			...
			if (isAccessible(GqamViewsRepository.General.Properties.scenario)) {
				// init part
				scenarioSettings = new EObjectFlatComboSettings(acquireStep, GqamPackage.eINSTANCE.getStep_Scenario());
			...
			if (isAccessible(GqamViewsRepository.General.Properties.childScenario)) {
				// init part
				childScenarioSettings = new EObjectFlatComboSettings(acquireStep, GqamPackage.eINSTANCE.getStep_ChildScenario());
			...
			if (isAccessible(GqamViewsRepository.General.Properties.inputRel)) {
				// init part
				inputRelSettings = new EObjectFlatComboSettings(acquireStep, GqamPackage.eINSTANCE.getStep_InputRel());
			...
			if (isAccessible(GqamViewsRepository.General.Properties.outputRel)) {
				// init part
				outputRelSettings = new EObjectFlatComboSettings(acquireStep, GqamPackage.eINSTANCE.getStep_OutputRel());
			...
			if (isAccessible(GqamViewsRepository.General.Properties.cause)) {
				causeSettings = new ReferencesTableSettings(acquireStep, GqamPackage.eINSTANCE.getBehaviorScenario_Cause());
				generalPart.initCause(causeSettings);
			}
			if (isAccessible(GqamViewsRepository.General.Properties.steps)) {
				stepsSettings = new ReferencesTableSettings(acquireStep, GqamPackage.eINSTANCE.getBehaviorScenario_Steps());
				generalPart.initSteps(stepsSettings);
			}
			if (isAccessible(GqamViewsRepository.General.Properties.connectors)) {
				connectorsSettings = new ReferencesTableSettings(acquireStep, GqamPackage.eINSTANCE.getBehaviorScenario_Connectors());
			...
			if (isAccessible(GqamViewsRepository.General.Properties.acqRes)) {
				generalPart.addFilterToAcqRes(new ViewerFilter() {
				...
				// Start of user code for additional businessfilters for acqRes
				// End of user code
			}
			if (isAccessible(GqamViewsRepository.General.Properties.concurRes)) {
	...
	public EStructuralFeature associatedFeature(Object editorKey) {
		...
		if (editorKey == GqamViewsRepository.General.Properties.resUnits) {
			return GqamPackage.eINSTANCE.getAcquireStep_ResUnits();
		}
		if (editorKey == GqamViewsRepository.General.Properties.acqRes) {
			return GqamPackage.eINSTANCE.getAcquireStep_AcqRes();
		}
	...
	public void updateSemanticModel(final IPropertiesEditionEvent event) {
		AcquireStep acquireStep = (AcquireStep)semanticObject;
		if (GqamViewsRepository.General.Properties.name == event.getAffectedEditor()) {
			acquireStep.setName((java.lang.String)EEFConverterUtil.createFromString(EcorePackage.Literals.ESTRING, (String)event.getNewValue()));
		}
		if (GqamViewsRepository.General.Properties.isAtomic == event.getAffectedEditor()) {
			acquireStep.setIsAtomic((Boolean)event.getNewValue());
		}
		if (GqamViewsRepository.General.Properties.repetitions == event.getAffectedEditor()) {
			acquireStep.setRepetitions((EEFConverterUtil.createDoubleFromString(EcorePackage.Literals.EDOUBLE, (String)event.getNewValue())));
		}
		if (GqamViewsRepository.General.Properties.probability == event.getAffectedEditor()) {
			acquireStep.setProbability((EEFConverterUtil.createDoubleFromString(EcorePackage.Literals.EDOUBLE, (String)event.getNewValue())));
		}
		if (GqamViewsRepository.General.Properties.priority == event.getAffectedEditor()) {
			acquireStep.setPriority((EEFConverterUtil.createIntFromString(EcorePackage.Literals.EINT, (String)event.getNewValue())));
		}
		if (GqamViewsRepository.General.Properties.serviceCount == event.getAffectedEditor()) {
			acquireStep.setServiceCount((EEFConverterUtil.createIntFromString(EcorePackage.Literals.EINT, (String)event.getNewValue())));
		}
		if (GqamViewsRepository.General.Properties.resUnits == event.getAffectedEditor()) {
			acquireStep.setResUnits((EEFConverterUtil.createIntFromString(EcorePackage.Literals.EINT, (String)event.getNewValue())));
		}
		if (GqamViewsRepository.General.Properties.acqRes == event.getAffectedEditor()) {
			if (event.getKind() == PropertiesEditionEvent.SET) {
				acqResSettings.setToReference((Resource)event.getNewValue());
			} else if (event.getKind() == PropertiesEditionEvent.ADD) {
				EReferencePropertiesEditionContext context = new EReferencePropertiesEditionContext(editingContext, this, acqResSettings, editingContext.getAdapterFactory());
	...
	public void updatePart(Notification msg) {
		...
		if (editingPart.isVisible()) {
			...
			if (GqamPackage.eINSTANCE.getAcquireStep_ResUnits().equals(msg.getFeature()) && msg.getNotifier().equals(semanticObject) && generalPart != null && isAccessible(GqamViewsRepository.General.Properties.resUnits)) {
				if (msg.getNewValue() != null) {
			...
			if (GqamPackage.eINSTANCE.getAcquireStep_AcqRes().equals(msg.getFeature()) && generalPart != null && isAccessible(GqamViewsRepository.General.Properties.acqRes))
				generalPart.setAcqRes((EObject)msg.getNewValue());
	...
	protected NotificationFilter[] getNotificationFilters() {
		NotificationFilter filter = new EStructuralFeatureNotificationFilter(
			...
			GqamPackage.eINSTANCE.getAcquireStep_ResUnits(),
			GqamPackage.eINSTANCE.getAcquireStep_AcqRes(),
		...
		if (event.getNewValue() != null) {
			try {
				...
				if (GqamViewsRepository.General.Properties.resUnits == event.getAffectedEditor()) {
					Object newValue = event.getNewValue();
					if (newValue instanceof String) {
						newValue = EEFConverterUtil.createFromString(GqamPackage.eINSTANCE.getAcquireStep_ResUnits().getEAttributeType(), (String)newValue);
					}
					ret = Diagnostician.INSTANCE.validate(GqamPackage.eINSTANCE.getAcquireStep_ResUnits().getEAttributeType(), newValue);
				}
