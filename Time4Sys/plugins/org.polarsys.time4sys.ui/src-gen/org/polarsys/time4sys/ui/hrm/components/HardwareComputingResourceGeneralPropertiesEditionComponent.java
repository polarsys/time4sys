 ...
 * 
 */
public class HardwareComputingResourceGeneralPropertiesEditionComponent extends SinglePartPropertiesEditingComponent {
	 ...
	 * 
	 */
	public HardwareComputingResourceGeneralPropertiesEditionComponent(PropertiesEditingContext editingContext, EObject hardwareComputingResource, String editing_mode) {
	...
	public void initPart(Object key, int kind, EObject elt, ResourceSet allResource) {
		...
		if (editingPart != null && key == partKey) {
			...
			if (isAccessible(HrmViewsRepository.General.Properties.name))
				generalPart.setName(EEFConverterUtil.convertToString(EcorePackage.Literals.ESTRING, hardwareComputingResource.getName()));
			
			if (isAccessible(HrmViewsRepository.General.Properties.isProtected)) {
				generalPart.setIsProtected(hardwareComputingResource.isIsProtected());
			}
			if (isAccessible(HrmViewsRepository.General.Properties.isActive)) {
				generalPart.setIsActive(hardwareComputingResource.isIsActive());
			}
			if (isAccessible(HrmViewsRepository.General.Properties.resMult)) {
				generalPart.setResMult(EEFConverterUtil.convertToString(EcorePackage.Literals.EINT, hardwareComputingResource.getResMult()));
			}
			
			if (isAccessible(HrmViewsRepository.General.Properties.speedFactor)) {
				generalPart.setSpeedFactor(EEFConverterUtil.convertToString(EcorePackage.Literals.EFLOAT, hardwareComputingResource.getSpeedFactor()));
			}
			
			if (isAccessible(HrmViewsRepository.General.Properties.mainScheduler)) {
				// init part
				mainSchedulerSettings = new EObjectFlatComboSettings(hardwareComputingResource, GrmPackage.eINSTANCE.getProcessingResource_MainScheduler());
			...
			if (isAccessible(HrmViewsRepository.General.Properties.ownedResource)) {
				ownedResourceSettings = new ReferencesTableSettings(hardwareComputingResource, GrmPackage.eINSTANCE.getResource_OwnedResource());
				generalPart.initOwnedResource(ownedResourceSettings);
			}
			if (isAccessible(HrmViewsRepository.General.Properties.ownedConnector)) {
				ownedConnectorSettings = new ReferencesTableSettings(hardwareComputingResource, GrmPackage.eINSTANCE.getResource_OwnedConnector());
				generalPart.initOwnedConnector(ownedConnectorSettings);
			}
			if (isAccessible(HrmViewsRepository.General.Properties.usedBy)) {
				usedBySettings = new ReferencesTableSettings(hardwareComputingResource, GrmPackage.eINSTANCE.getResource_UsedBy());
	...
	public EStructuralFeature associatedFeature(Object editorKey) {
		...
		if (editorKey == HrmViewsRepository.General.Properties.speedFactor) {
			return GrmPackage.eINSTANCE.getProcessingResource_SpeedFactor();
		}
		if (editorKey == HrmViewsRepository.General.Properties.mainScheduler) {
	...
	public void updateSemanticModel(final IPropertiesEditionEvent event) {
		...
		if (HrmViewsRepository.General.Properties.name == event.getAffectedEditor()) {
			hardwareComputingResource.setName((java.lang.String)EEFConverterUtil.createFromString(EcorePackage.Literals.ESTRING, (String)event.getNewValue()));
		}
		if (HrmViewsRepository.General.Properties.isProtected == event.getAffectedEditor()) {
			hardwareComputingResource.setIsProtected((Boolean)event.getNewValue());
		}
		if (HrmViewsRepository.General.Properties.isActive == event.getAffectedEditor()) {
			hardwareComputingResource.setIsActive((Boolean)event.getNewValue());
		}
		if (HrmViewsRepository.General.Properties.resMult == event.getAffectedEditor()) {
			hardwareComputingResource.setResMult((EEFConverterUtil.createIntFromString(EcorePackage.Literals.EINT, (String)event.getNewValue())));
		}
		if (HrmViewsRepository.General.Properties.speedFactor == event.getAffectedEditor()) {
			hardwareComputingResource.setSpeedFactor((EEFConverterUtil.createFloatFromString(EcorePackage.Literals.EFLOAT, (String)event.getNewValue())));
		}
		if (HrmViewsRepository.General.Properties.mainScheduler == event.getAffectedEditor()) {
	...
	public boolean mustBeComposed(Object key, int kind) {
		return key == HrmViewsRepository.General.Properties.name || key == HrmViewsRepository.General.Properties.isProtected || key == HrmViewsRepository.General.Properties.isActive || key == HrmViewsRepository.General.Properties.resMult || key == HrmViewsRepository.General.Properties.speedFactor || key == HrmViewsRepository.General.Properties.mainScheduler || key == HrmViewsRepository.General.Properties.ownedResource || key == HrmViewsRepository.General.Properties.ownedConnector || key == HrmViewsRepository.General.Properties.usedBy || key == HrmViewsRepository.General.Properties.class;
	}
