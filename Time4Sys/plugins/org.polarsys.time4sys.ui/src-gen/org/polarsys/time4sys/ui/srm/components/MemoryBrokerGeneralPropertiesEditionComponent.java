 ...
 * 
 */
public class MemoryBrokerGeneralPropertiesEditionComponent extends SinglePartPropertiesEditingComponent {
	...
	/**
	 * Settings for memories ReferencesTable
	 */
	private ReferencesTableSettings memoriesSettings;
	 ...
	 * 
	 */
	public MemoryBrokerGeneralPropertiesEditionComponent(PropertiesEditingContext editingContext, EObject memoryBroker, String editing_mode) {
	...
	public void initPart(Object key, int kind, EObject elt, ResourceSet allResource) {
		...
		if (editingPart != null && key == partKey) {
			...
			if (isAccessible(SrmViewsRepository.General.Properties.name))
				generalPart.setName(EEFConverterUtil.convertToString(EcorePackage.Literals.ESTRING, memoryBroker.getName()));
			
			if (isAccessible(SrmViewsRepository.General.Properties.ownedResource)) {
				ownedResourceSettings = new ReferencesTableSettings(memoryBroker, GrmPackage.eINSTANCE.getResource_OwnedResource());
				generalPart.initOwnedResource(ownedResourceSettings);
			}
			if (isAccessible(SrmViewsRepository.General.Properties.resMult)) {
				generalPart.setResMult(EEFConverterUtil.convertToString(EcorePackage.Literals.EINT, memoryBroker.getResMult()));
			}
			
			if (isAccessible(SrmViewsRepository.General.Properties.isProtected)) {
				generalPart.setIsProtected(memoryBroker.isIsProtected());
			}
			if (isAccessible(SrmViewsRepository.General.Properties.isActive)) {
				generalPart.setIsActive(memoryBroker.isIsActive());
			}
			if (isAccessible(SrmViewsRepository.General.Properties.ownedPort)) {
				ownedPortSettings = new ReferencesTableSettings(memoryBroker, GrmPackage.eINSTANCE.getResource_OwnedPort());
				generalPart.initOwnedPort(ownedPortSettings);
			}
			if (isAccessible(SrmViewsRepository.General.Properties.ownedConnector)) {
				ownedConnectorSettings = new ReferencesTableSettings(memoryBroker, GrmPackage.eINSTANCE.getResource_OwnedConnector());
				generalPart.initOwnedConnector(ownedConnectorSettings);
			}
			if (isAccessible(SrmViewsRepository.General.Properties.usedBy)) {
				usedBySettings = new ReferencesTableSettings(memoryBroker, GrmPackage.eINSTANCE.getResource_UsedBy());
				generalPart.initUsedBy(usedBySettings);
			}
			if (isAccessible(SrmViewsRepository.General.Properties.brokedResource)) {
				brokedResourceSettings = new ReferencesTableSettings(memoryBroker, GrmPackage.eINSTANCE.getResourceBroker_BrokedResource());
			...
			if (isAccessible(SrmViewsRepository.General.Properties.managedResource)) {
				managedResourceSettings = new ReferencesTableSettings(memoryBroker, GrmPackage.eINSTANCE.getResourceManager_ManagedResource());
			...
			if (isAccessible(SrmViewsRepository.General.Properties.memorySizeFootprint)) {
				generalPart.setMemorySizeFootprint(EEFConverterUtil.convertToString(EcorePackage.Literals.EINT, memoryBroker.getMemorySizeFootprint()));
			}
			
			if (isAccessible(SrmViewsRepository.General.Properties.stateElements))
				generalPart.setStateElements(memoryBroker.getStateElements());
			
			if (isAccessible(SrmViewsRepository.General.Properties.identifierElements))
				generalPart.setIdentifierElements(memoryBroker.getIdentifierElements());
			
			if (isAccessible(SrmViewsRepository.General.Properties.memories)) {
				memoriesSettings = new ReferencesTableSettings(memoryBroker, SrmPackage.eINSTANCE.getMemoryBroker_Memories());
				generalPart.initMemories(memoriesSettings);
			}
			// init filters
			...
			if (isAccessible(SrmViewsRepository.General.Properties.memories)) {
				generalPart.addFilterToMemories(new EObjectFilter(GrmPackage.Literals.STORAGE_RESOURCE));
				// Start of user code for additional businessfilters for memories
				// End of user code
			}
	...
	public EStructuralFeature associatedFeature(Object editorKey) {
		...
		if (editorKey == SrmViewsRepository.General.Properties.memories) {
			return SrmPackage.eINSTANCE.getMemoryBroker_Memories();
		}
	...
	public void updateSemanticModel(final IPropertiesEditionEvent event) {
		...
		if (SrmViewsRepository.General.Properties.name == event.getAffectedEditor()) {
			memoryBroker.setName((java.lang.String)EEFConverterUtil.createFromString(EcorePackage.Literals.ESTRING, (String)event.getNewValue()));
		}
		...
		if (SrmViewsRepository.General.Properties.resMult == event.getAffectedEditor()) {
			memoryBroker.setResMult((EEFConverterUtil.createIntFromString(EcorePackage.Literals.EINT, (String)event.getNewValue())));
		}
		if (SrmViewsRepository.General.Properties.isProtected == event.getAffectedEditor()) {
			memoryBroker.setIsProtected((Boolean)event.getNewValue());
		}
		if (SrmViewsRepository.General.Properties.isActive == event.getAffectedEditor()) {
			memoryBroker.setIsActive((Boolean)event.getNewValue());
		}
		...
		if (SrmViewsRepository.General.Properties.memorySizeFootprint == event.getAffectedEditor()) {
			memoryBroker.setMemorySizeFootprint((EEFConverterUtil.createIntFromString(EcorePackage.Literals.EINT, (String)event.getNewValue())));
		}
		if (SrmViewsRepository.General.Properties.stateElements == event.getAffectedEditor()) {
			if (event.getKind() == PropertiesEditionEvent.SET) {
				memoryBroker.getStateElements().clear();
				memoryBroker.getStateElements().addAll(((EList) event.getNewValue()));
			}
		}
		if (SrmViewsRepository.General.Properties.identifierElements == event.getAffectedEditor()) {
			if (event.getKind() == PropertiesEditionEvent.SET) {
				memoryBroker.getIdentifierElements().clear();
				memoryBroker.getIdentifierElements().addAll(((EList) event.getNewValue()));
			}
		}
		if (SrmViewsRepository.General.Properties.memories == event.getAffectedEditor()) {
			if (event.getKind() == PropertiesEditionEvent.ADD) {
				if (event.getNewValue() instanceof StorageResource) {
					memoriesSettings.addToReference((EObject) event.getNewValue());
				}
			} else if (event.getKind() == PropertiesEditionEvent.REMOVE) {
				memoriesSettings.removeFromReference((EObject) event.getNewValue());
			} else if (event.getKind() == PropertiesEditionEvent.MOVE) {
				memoriesSettings.move(event.getNewIndex(), (StorageResource) event.getNewValue());
			}
		}
	}
	...
	public void updatePart(Notification msg) {
		...
		if (editingPart.isVisible()) {
			...
			if (SrmPackage.eINSTANCE.getMemoryBroker_Memories().equals(msg.getFeature())  && isAccessible(SrmViewsRepository.General.Properties.memories))
				generalPart.updateMemories();
	...
	protected NotificationFilter[] getNotificationFilters() {
		NotificationFilter filter = new EStructuralFeatureNotificationFilter(
			...
			SrmPackage.eINSTANCE.getMemoryBroker_Memories()		);
