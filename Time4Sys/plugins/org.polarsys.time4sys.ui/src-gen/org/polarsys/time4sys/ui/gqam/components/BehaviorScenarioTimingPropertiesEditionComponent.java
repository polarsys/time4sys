 ...
 * 
 */
public class BehaviorScenarioTimingPropertiesEditionComponent extends SinglePartPropertiesEditingComponent {
	 ...
	 * 
	 */
	public BehaviorScenarioTimingPropertiesEditionComponent(PropertiesEditingContext editingContext, EObject behaviorScenario, String editing_mode) {
	...
	public void initPart(Object key, int kind, EObject elt, ResourceSet allResource) {
		...
		if (editingPart != null && key == partKey) {
			...
			if (isAccessible(GqamViewsRepository.Timing.TimingProperties.respTime))
				timingPart.setRespTime(EEFConverterUtil.convertToString(GqamPackage.Literals.NFP_DURATION, behaviorScenario.getRespTime()));
			
			if (isAccessible(GqamViewsRepository.Timing.TimingProperties.bestCET))
				timingPart.setBestCET(EEFConverterUtil.convertToString(GqamPackage.Literals.NFP_DURATION, behaviorScenario.getBestCET()));
			
			if (isAccessible(GqamViewsRepository.Timing.TimingProperties.worstCET))
				timingPart.setWorstCET(EEFConverterUtil.convertToString(GqamPackage.Literals.NFP_DURATION, behaviorScenario.getWorstCET()));
	...
	public void updateSemanticModel(final IPropertiesEditionEvent event) {
		...
		if (GqamViewsRepository.Timing.TimingProperties.respTime == event.getAffectedEditor()) {
			behaviorScenario.setRespTime((org.polarsys.time4sys.marte.nfp.Duration)EEFConverterUtil.createFromString(GqamPackage.Literals.NFP_DURATION, (String)event.getNewValue()));
		}
		if (GqamViewsRepository.Timing.TimingProperties.bestCET == event.getAffectedEditor()) {
			behaviorScenario.setBestCET((org.polarsys.time4sys.marte.nfp.Duration)EEFConverterUtil.createFromString(GqamPackage.Literals.NFP_DURATION, (String)event.getNewValue()));
		}
		if (GqamViewsRepository.Timing.TimingProperties.worstCET == event.getAffectedEditor()) {
			behaviorScenario.setWorstCET((org.polarsys.time4sys.marte.nfp.Duration)EEFConverterUtil.createFromString(GqamPackage.Literals.NFP_DURATION, (String)event.getNewValue()));
		}
	}
	...
	public boolean mustBeComposed(Object key, int kind) {
		return key == GqamViewsRepository.Timing.TimingProperties.respTime || key == GqamViewsRepository.Timing.TimingProperties.bestCET || key == GqamViewsRepository.Timing.TimingProperties.worstCET || key == GqamViewsRepository.Timing.TimingProperties.class;
	}
