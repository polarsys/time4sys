 ...
 * 
 */
public class HardwareInterfaceOtherPropertiesEditionComponent extends SinglePartPropertiesEditingComponent {
	 ...
	 * 
	 */
	public HardwareInterfaceOtherPropertiesEditionComponent(PropertiesEditingContext editingContext, EObject hardwareInterface, String editing_mode) {
		super(editingContext, hardwareInterface, editing_mode);
	...
	public void initPart(Object key, int kind, EObject elt, ResourceSet allResource) {
		...
		if (editingPart != null && key == partKey) {
			...
			final HardwareInterface hardwareInterface = (HardwareInterface)elt;
			final OtherPropertiesEditionPart otherPart = (OtherPropertiesEditionPart)editingPart;
			// init values
			if (isAccessible(HrmViewsRepository.Other.Properties.ownedService)) {
				ownedServiceSettings = new ReferencesTableSettings(hardwareInterface, GrmPackage.eINSTANCE.getResourceInterface_OwnedService());
				otherPart.initOwnedService(ownedServiceSettings);
			}
			// init filters
			if (isAccessible(HrmViewsRepository.Other.Properties.ownedService)) {
				otherPart.addFilterToOwnedService(new ViewerFilter() {
					/**
					 * {@inheritDoc}
					 * 
	...
	public EStructuralFeature associatedFeature(Object editorKey) {
		if (editorKey == HrmViewsRepository.Other.Properties.ownedService) {
	...
	public void updateSemanticModel(final IPropertiesEditionEvent event) {
		HardwareInterface hardwareInterface = (HardwareInterface)semanticObject;
		if (HrmViewsRepository.Other.Properties.ownedService == event.getAffectedEditor()) {
	...
	public void updatePart(Notification msg) {
		...
		if (editingPart.isVisible()) {
			...
			if (GrmPackage.eINSTANCE.getResourceInterface_OwnedService().equals(msg.getFeature()) && isAccessible(HrmViewsRepository.Other.Properties.ownedService))
				otherPart.updateOwnedService();
	...
	public boolean mustBeComposed(Object key, int kind) {
		return key == HrmViewsRepository.Other.Properties.ownedService || key == HrmViewsRepository.Other.Properties.class;
	}
