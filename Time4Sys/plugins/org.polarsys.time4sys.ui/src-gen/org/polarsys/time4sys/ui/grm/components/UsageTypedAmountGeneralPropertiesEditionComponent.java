 ...
 * 
 */
public class UsageTypedAmountGeneralPropertiesEditionComponent extends SinglePartPropertiesEditingComponent {
	 ...
	 * 
	 */
	public UsageTypedAmountGeneralPropertiesEditionComponent(PropertiesEditingContext editingContext, EObject usageTypedAmount, String editing_mode) {
	...
	public void initPart(Object key, int kind, EObject elt, ResourceSet allResource) {
		...
		if (editingPart != null && key == partKey) {
			...
			if (isAccessible(GrmViewsRepository.General.Properties.name))
				generalPart.setName(EEFConverterUtil.convertToString(EcorePackage.Literals.ESTRING, usageTypedAmount.getName()));
			
			if (isAccessible(GrmViewsRepository.General.Properties.ownedResource)) {
				ownedResourceSettings = new ReferencesTableSettings(usageTypedAmount, GrmPackage.eINSTANCE.getResource_OwnedResource());
				generalPart.initOwnedResource(ownedResourceSettings);
			}
			if (isAccessible(GrmViewsRepository.General.Properties.resMult)) {
				generalPart.setResMult(EEFConverterUtil.convertToString(EcorePackage.Literals.EINT, usageTypedAmount.getResMult()));
			}
			
			if (isAccessible(GrmViewsRepository.General.Properties.isProtected)) {
				generalPart.setIsProtected(usageTypedAmount.isIsProtected());
			}
			if (isAccessible(GrmViewsRepository.General.Properties.isActive)) {
				generalPart.setIsActive(usageTypedAmount.isIsActive());
			}
			if (isAccessible(GrmViewsRepository.General.Properties.ownedPort)) {
				ownedPortSettings = new ReferencesTableSettings(usageTypedAmount, GrmPackage.eINSTANCE.getResource_OwnedPort());
				generalPart.initOwnedPort(ownedPortSettings);
			}
			if (isAccessible(GrmViewsRepository.General.Properties.ownedConnector)) {
				ownedConnectorSettings = new ReferencesTableSettings(usageTypedAmount, GrmPackage.eINSTANCE.getResource_OwnedConnector());
				generalPart.initOwnedConnector(ownedConnectorSettings);
			}
			if (isAccessible(GrmViewsRepository.General.Properties.usedBy)) {
				usedBySettings = new ReferencesTableSettings(usageTypedAmount, GrmPackage.eINSTANCE.getResource_UsedBy());
				generalPart.initUsedBy(usedBySettings);
			}
			if (isAccessible(GrmViewsRepository.General.Properties.msgSize)) {
				generalPart.setMsgSize(EEFConverterUtil.convertToString(EcorePackage.Literals.EINT, usageTypedAmount.getMsgSize()));
			}
	...
	public EStructuralFeature associatedFeature(Object editorKey) {
		...
		if (editorKey == GrmViewsRepository.General.Properties.msgSize) {
			return GrmPackage.eINSTANCE.getUsageTypedAmount_MsgSize();
		}
	...
	public void updateSemanticModel(final IPropertiesEditionEvent event) {
		...
		if (GrmViewsRepository.General.Properties.name == event.getAffectedEditor()) {
			usageTypedAmount.setName((java.lang.String)EEFConverterUtil.createFromString(EcorePackage.Literals.ESTRING, (String)event.getNewValue()));
		}
		...
		if (GrmViewsRepository.General.Properties.resMult == event.getAffectedEditor()) {
			usageTypedAmount.setResMult((EEFConverterUtil.createIntFromString(EcorePackage.Literals.EINT, (String)event.getNewValue())));
		}
		if (GrmViewsRepository.General.Properties.isProtected == event.getAffectedEditor()) {
			usageTypedAmount.setIsProtected((Boolean)event.getNewValue());
		}
		if (GrmViewsRepository.General.Properties.isActive == event.getAffectedEditor()) {
			usageTypedAmount.setIsActive((Boolean)event.getNewValue());
		}
		...
		if (GrmViewsRepository.General.Properties.msgSize == event.getAffectedEditor()) {
			usageTypedAmount.setMsgSize((EEFConverterUtil.createIntFromString(EcorePackage.Literals.EINT, (String)event.getNewValue())));
		}
	}
	...
	public void updatePart(Notification msg) {
		...
		if (editingPart.isVisible()) {
			...
			if (GrmPackage.eINSTANCE.getUsageTypedAmount_MsgSize().equals(msg.getFeature()) && msg.getNotifier().equals(semanticObject) && generalPart != null && isAccessible(GrmViewsRepository.General.Properties.msgSize)) {
				if (msg.getNewValue() != null) {
	...
	protected NotificationFilter[] getNotificationFilters() {
		NotificationFilter filter = new EStructuralFeatureNotificationFilter(
			...
			GrmPackage.eINSTANCE.getUsageTypedAmount_MsgSize()		);
		...
		if (event.getNewValue() != null) {
			try {
				...
				if (GrmViewsRepository.General.Properties.msgSize == event.getAffectedEditor()) {
					Object newValue = event.getNewValue();
					if (newValue instanceof String) {
						newValue = EEFConverterUtil.createFromString(GrmPackage.eINSTANCE.getUsageTypedAmount_MsgSize().getEAttributeType(), (String)newValue);
					}
					ret = Diagnostician.INSTANCE.validate(GrmPackage.eINSTANCE.getUsageTypedAmount_MsgSize().getEAttributeType(), newValue);
				}
