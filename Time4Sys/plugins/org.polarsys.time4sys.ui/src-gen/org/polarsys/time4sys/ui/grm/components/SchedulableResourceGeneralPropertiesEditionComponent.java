 ...
 * 
 */
public class SchedulableResourceGeneralPropertiesEditionComponent extends SinglePartPropertiesEditingComponent {
	 ...
	 * 
	 */
	public SchedulableResourceGeneralPropertiesEditionComponent(PropertiesEditingContext editingContext, EObject schedulableResource, String editing_mode) {
		super(editingContext, schedulableResource, editing_mode);
	...
	public void initPart(Object key, int kind, EObject elt, ResourceSet allResource) {
		...
		if (editingPart != null && key == partKey) {
			...
			final SchedulableResource schedulableResource = (SchedulableResource)elt;
			final GeneralPropertiesEditionPart generalPart = (GeneralPropertiesEditionPart)editingPart;
			// init values
			if (isAccessible(GrmViewsRepository.General.Properties.name))
				generalPart.setName(EEFConverterUtil.convertToString(EcorePackage.Literals.ESTRING, schedulableResource.getName()));
			
			if (isAccessible(GrmViewsRepository.General.Properties.ownedResource)) {
				ownedResourceSettings = new ReferencesTableSettings(schedulableResource, GrmPackage.eINSTANCE.getResource_OwnedResource());
				generalPart.initOwnedResource(ownedResourceSettings);
			}
			if (isAccessible(GrmViewsRepository.General.Properties.resMult)) {
				generalPart.setResMult(EEFConverterUtil.convertToString(EcorePackage.Literals.EINT, schedulableResource.getResMult()));
			}
			
			if (isAccessible(GrmViewsRepository.General.Properties.isProtected)) {
				generalPart.setIsProtected(schedulableResource.isIsProtected());
			}
			if (isAccessible(GrmViewsRepository.General.Properties.isActive)) {
				generalPart.setIsActive(schedulableResource.isIsActive());
			}
			if (isAccessible(GrmViewsRepository.General.Properties.ownedPort)) {
				ownedPortSettings = new ReferencesTableSettings(schedulableResource, GrmPackage.eINSTANCE.getResource_OwnedPort());
				generalPart.initOwnedPort(ownedPortSettings);
			}
			if (isAccessible(GrmViewsRepository.General.Properties.ownedConnector)) {
				ownedConnectorSettings = new ReferencesTableSettings(schedulableResource, GrmPackage.eINSTANCE.getResource_OwnedConnector());
				generalPart.initOwnedConnector(ownedConnectorSettings);
			}
			if (isAccessible(GrmViewsRepository.General.Properties.usedBy)) {
				usedBySettings = new ReferencesTableSettings(schedulableResource, GrmPackage.eINSTANCE.getResource_UsedBy());
				generalPart.initUsedBy(usedBySettings);
			}
			if (isAccessible(GrmViewsRepository.General.Properties.dependentScheduler)) {
				// init part
				dependentSchedulerSettings = new EObjectFlatComboSettings(schedulableResource, GrmPackage.eINSTANCE.getSchedulableResource_DependentScheduler());
			...
			if (isAccessible(GrmViewsRepository.General.Properties.host)) {
				// init part
				hostSettings = new EObjectFlatComboSettings(schedulableResource, GrmPackage.eINSTANCE.getSchedulableResource_Host());
				generalPart.initHost(hostSettings);
				// set the button mode
				generalPart.setHostButtonMode(ButtonsModeEnum.BROWSE);
			}
			if (isAccessible(GrmViewsRepository.General.Properties.schedParams)) {
				schedParamsSettings = new ReferencesTableSettings(schedulableResource, GrmPackage.eINSTANCE.getSchedulableResource_SchedParams());
	...
	public EStructuralFeature associatedFeature(Object editorKey) {
		...
		if (editorKey == GrmViewsRepository.General.Properties.dependentScheduler) {
		...
		if (editorKey == GrmViewsRepository.General.Properties.schedParams) {
	...
	public void updateSemanticModel(final IPropertiesEditionEvent event) {
		SchedulableResource schedulableResource = (SchedulableResource)semanticObject;
		if (GrmViewsRepository.General.Properties.name == event.getAffectedEditor()) {
			schedulableResource.setName((java.lang.String)EEFConverterUtil.createFromString(EcorePackage.Literals.ESTRING, (String)event.getNewValue()));
		}
		...
		if (GrmViewsRepository.General.Properties.resMult == event.getAffectedEditor()) {
			schedulableResource.setResMult((EEFConverterUtil.createIntFromString(EcorePackage.Literals.EINT, (String)event.getNewValue())));
		}
		if (GrmViewsRepository.General.Properties.isProtected == event.getAffectedEditor()) {
			schedulableResource.setIsProtected((Boolean)event.getNewValue());
		}
		if (GrmViewsRepository.General.Properties.isActive == event.getAffectedEditor()) {
			schedulableResource.setIsActive((Boolean)event.getNewValue());
		}
		...
		if (GrmViewsRepository.General.Properties.dependentScheduler == event.getAffectedEditor()) {
		...
		if (GrmViewsRepository.General.Properties.schedParams == event.getAffectedEditor()) {
			if (event.getKind() == PropertiesEditionEvent.ADD) {
	...
	public void updatePart(Notification msg) {
		...
		if (editingPart.isVisible()) {
			...
			if (GrmPackage.eINSTANCE.getSchedulableResource_DependentScheduler().equals(msg.getFeature()) && generalPart != null && isAccessible(GrmViewsRepository.General.Properties.dependentScheduler))
				generalPart.setDependentScheduler((EObject)msg.getNewValue());
			if (GrmPackage.eINSTANCE.getSchedulableResource_Host().equals(msg.getFeature()) && generalPart != null && isAccessible(GrmViewsRepository.General.Properties.host))
				generalPart.setHost((EObject)msg.getNewValue());
			if (GrmPackage.eINSTANCE.getSchedulableResource_SchedParams().equals(msg.getFeature()) && isAccessible(GrmViewsRepository.General.Properties.schedParams))
	...
	public boolean isRequired(Object key, int kind) {
		return key == GrmViewsRepository.General.Properties.host;
	}
