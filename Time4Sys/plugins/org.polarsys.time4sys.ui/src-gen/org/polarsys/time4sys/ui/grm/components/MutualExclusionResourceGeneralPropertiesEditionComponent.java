 ...
 * 
 */
public class MutualExclusionResourceGeneralPropertiesEditionComponent extends SinglePartPropertiesEditingComponent {
	 ...
	 * 
	 */
	public MutualExclusionResourceGeneralPropertiesEditionComponent(PropertiesEditingContext editingContext, EObject mutualExclusionResource, String editing_mode) {
	...
	public void initPart(Object key, int kind, EObject elt, ResourceSet allResource) {
		...
		if (editingPart != null && key == partKey) {
			...
			if (isAccessible(GrmViewsRepository.General.Properties.name))
				generalPart.setName(EEFConverterUtil.convertToString(EcorePackage.Literals.ESTRING, mutualExclusionResource.getName()));
			
			if (isAccessible(GrmViewsRepository.General.Properties.ownedResource)) {
				ownedResourceSettings = new ReferencesTableSettings(mutualExclusionResource, GrmPackage.eINSTANCE.getResource_OwnedResource());
				generalPart.initOwnedResource(ownedResourceSettings);
			}
			if (isAccessible(GrmViewsRepository.General.Properties.resMult)) {
				generalPart.setResMult(EEFConverterUtil.convertToString(EcorePackage.Literals.EINT, mutualExclusionResource.getResMult()));
			}
			
			if (isAccessible(GrmViewsRepository.General.Properties.isPeriodic)) {
				generalPart.setIsPeriodic(mutualExclusionResource.isIsProtected());
			}
			if (isAccessible(GrmViewsRepository.General.Properties.isActive)) {
				generalPart.setIsActive(mutualExclusionResource.isIsActive());
			}
			if (isAccessible(GrmViewsRepository.General.Properties.ownedPort)) {
				ownedPortSettings = new ReferencesTableSettings(mutualExclusionResource, GrmPackage.eINSTANCE.getResource_OwnedPort());
				generalPart.initOwnedPort(ownedPortSettings);
			}
			if (isAccessible(GrmViewsRepository.General.Properties.ownedConnector)) {
				ownedConnectorSettings = new ReferencesTableSettings(mutualExclusionResource, GrmPackage.eINSTANCE.getResource_OwnedConnector());
				generalPart.initOwnedConnector(ownedConnectorSettings);
			}
			if (isAccessible(GrmViewsRepository.General.Properties.usedBy)) {
				usedBySettings = new ReferencesTableSettings(mutualExclusionResource, GrmPackage.eINSTANCE.getResource_UsedBy());
				generalPart.initUsedBy(usedBySettings);
			}
			if (isAccessible(GrmViewsRepository.General.Properties.scheduler)) {
				// init part
				schedulerSettings = new EObjectFlatComboSettings(mutualExclusionResource, GrmPackage.eINSTANCE.getMutualExclusionResource_Scheduler());
				generalPart.initScheduler(schedulerSettings);
				// set the button mode
				generalPart.setSchedulerButtonMode(ButtonsModeEnum.BROWSE);
			}
			if (isAccessible(GrmViewsRepository.General.Properties.protectParams)) {
				protectParamsSettings = new ReferencesTableSettings(mutualExclusionResource, GrmPackage.eINSTANCE.getMutualExclusionResource_ProtectParams());
				generalPart.initProtectParams(protectParamsSettings);
			}
			// init filters
			...
			if (isAccessible(GrmViewsRepository.General.Properties.protectParams)) {
				generalPart.addFilterToProtectParams(new ViewerFilter() {
					/**
					 * {@inheritDoc}
					 * 
					 * @see org.eclipse.jface.viewers.ViewerFilter#select(org.eclipse.jface.viewers.Viewer, java.lang.Object, java.lang.Object)
	...
	public EStructuralFeature associatedFeature(Object editorKey) {
		...
		if (editorKey == GrmViewsRepository.General.Properties.scheduler) {
			return GrmPackage.eINSTANCE.getMutualExclusionResource_Scheduler();
		}
		if (editorKey == GrmViewsRepository.General.Properties.protectParams) {
	...
	public void updateSemanticModel(final IPropertiesEditionEvent event) {
		...
		if (GrmViewsRepository.General.Properties.resMult == event.getAffectedEditor()) {
			mutualExclusionResource.setResMult((EEFConverterUtil.createIntFromString(EcorePackage.Literals.EINT, (String)event.getNewValue())));
		}
		if (GrmViewsRepository.General.Properties.isPeriodic == event.getAffectedEditor()) {
			mutualExclusionResource.setIsProtected((Boolean)event.getNewValue());
		}
		if (GrmViewsRepository.General.Properties.isActive == event.getAffectedEditor()) {
			mutualExclusionResource.setIsActive((Boolean)event.getNewValue());
		}
		...
		if (GrmViewsRepository.General.Properties.scheduler == event.getAffectedEditor()) {
		...
		if (GrmViewsRepository.General.Properties.protectParams == event.getAffectedEditor()) {
			if (event.getKind() == PropertiesEditionEvent.ADD) {
