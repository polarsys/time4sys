 ...
 * 
 */
public class ReleaseStepGeneralPropertiesEditionComponent extends SinglePartPropertiesEditingComponent {
	...
	/**
	 * Settings for relRes EObjectFlatComboViewer
	 */
	private EObjectFlatComboSettings relResSettings;
	 ...
	 * 
	 */
	public ReleaseStepGeneralPropertiesEditionComponent(PropertiesEditingContext editingContext, EObject releaseStep, String editing_mode) {
		super(editingContext, releaseStep, editing_mode);
	...
	public void initPart(Object key, int kind, EObject elt, ResourceSet allResource) {
		...
		if (editingPart != null && key == partKey) {
			...
			final ReleaseStep releaseStep = (ReleaseStep)elt;
			final GeneralPropertiesEditionPart generalPart = (GeneralPropertiesEditionPart)editingPart;
			// init values
			if (isAccessible(GqamViewsRepository.General.Properties.name))
				generalPart.setName(EEFConverterUtil.convertToString(EcorePackage.Literals.ESTRING, releaseStep.getName()));
			
			if (isAccessible(GqamViewsRepository.General.Properties.isAtomic)) {
				generalPart.setIsAtomic(releaseStep.isIsAtomic());
			}
			if (isAccessible(GqamViewsRepository.General.Properties.repetitions)) {
				generalPart.setRepetitions(EEFConverterUtil.convertToString(EcorePackage.Literals.EDOUBLE, releaseStep.getRepetitions()));
			}
			
			if (isAccessible(GqamViewsRepository.General.Properties.probability)) {
				generalPart.setProbability(EEFConverterUtil.convertToString(EcorePackage.Literals.EDOUBLE, releaseStep.getProbability()));
			}
			
			if (isAccessible(GqamViewsRepository.General.Properties.priority)) {
				generalPart.setPriority(EEFConverterUtil.convertToString(EcorePackage.Literals.EINT, releaseStep.getPriority()));
			}
			
			if (isAccessible(GqamViewsRepository.General.Properties.serviceCount)) {
				generalPart.setServiceCount(EEFConverterUtil.convertToString(EcorePackage.Literals.EINT, releaseStep.getServiceCount()));
			}
			
			if (isAccessible(GqamViewsRepository.General.Properties.resUnits)) {
				generalPart.setResUnits(EEFConverterUtil.convertToString(EcorePackage.Literals.EINT, releaseStep.getResUnits()));
			}
			
			if (isAccessible(GqamViewsRepository.General.Properties.relRes)) {
				// init part
				relResSettings = new EObjectFlatComboSettings(releaseStep, GqamPackage.eINSTANCE.getReleaseStep_RelRes());
				generalPart.initRelRes(relResSettings);
				// set the button mode
				generalPart.setRelResButtonMode(ButtonsModeEnum.BROWSE);
			}
			if (isAccessible(GqamViewsRepository.General.Properties.root)) {
				// init part
				rootSettings = new EObjectFlatComboSettings(releaseStep, GqamPackage.eINSTANCE.getBehaviorScenario_Root());
			...
			if (isAccessible(GqamViewsRepository.General.Properties.parentStep)) {
				// init part
				parentStepSettings = new EObjectFlatComboSettings(releaseStep, GqamPackage.eINSTANCE.getBehaviorScenario_ParentStep());
			...
			if (isAccessible(GqamViewsRepository.General.Properties.scenario)) {
				// init part
				scenarioSettings = new EObjectFlatComboSettings(releaseStep, GqamPackage.eINSTANCE.getStep_Scenario());
			...
			if (isAccessible(GqamViewsRepository.General.Properties.childScenario)) {
				// init part
				childScenarioSettings = new EObjectFlatComboSettings(releaseStep, GqamPackage.eINSTANCE.getStep_ChildScenario());
			...
			if (isAccessible(GqamViewsRepository.General.Properties.inputRel)) {
				// init part
				inputRelSettings = new EObjectFlatComboSettings(releaseStep, GqamPackage.eINSTANCE.getStep_InputRel());
			...
			if (isAccessible(GqamViewsRepository.General.Properties.outputRel)) {
				// init part
				outputRelSettings = new EObjectFlatComboSettings(releaseStep, GqamPackage.eINSTANCE.getStep_OutputRel());
			...
			if (isAccessible(GqamViewsRepository.General.Properties.concurRes)) {
				// init part
				concurResSettings = new EObjectFlatComboSettings(releaseStep, GqamPackage.eINSTANCE.getStep_ConcurRes());
			...
			if (isAccessible(GqamViewsRepository.General.Properties.steps)) {
				stepsSettings = new ReferencesTableSettings(releaseStep, GqamPackage.eINSTANCE.getBehaviorScenario_Steps());
				generalPart.initSteps(stepsSettings);
			}
			if (isAccessible(GqamViewsRepository.General.Properties.cause)) {
				causeSettings = new ReferencesTableSettings(releaseStep, GqamPackage.eINSTANCE.getBehaviorScenario_Cause());
				generalPart.initCause(causeSettings);
			}
			if (isAccessible(GqamViewsRepository.General.Properties.connectors)) {
				connectorsSettings = new ReferencesTableSettings(releaseStep, GqamPackage.eINSTANCE.getBehaviorScenario_Connectors());
			...
			if (isAccessible(GqamViewsRepository.General.Properties.relRes)) {
				generalPart.addFilterToRelRes(new ViewerFilter() {
				...
				// Start of user code for additional businessfilters for relRes
				// End of user code
			}
			if (isAccessible(GqamViewsRepository.General.Properties.root)) {
	...
	public EStructuralFeature associatedFeature(Object editorKey) {
		...
		if (editorKey == GqamViewsRepository.General.Properties.resUnits) {
			return GqamPackage.eINSTANCE.getReleaseStep_ResUnits();
		}
		if (editorKey == GqamViewsRepository.General.Properties.relRes) {
			return GqamPackage.eINSTANCE.getReleaseStep_RelRes();
		}
	...
	public void updateSemanticModel(final IPropertiesEditionEvent event) {
		ReleaseStep releaseStep = (ReleaseStep)semanticObject;
		if (GqamViewsRepository.General.Properties.name == event.getAffectedEditor()) {
			releaseStep.setName((java.lang.String)EEFConverterUtil.createFromString(EcorePackage.Literals.ESTRING, (String)event.getNewValue()));
		}
		if (GqamViewsRepository.General.Properties.isAtomic == event.getAffectedEditor()) {
			releaseStep.setIsAtomic((Boolean)event.getNewValue());
		}
		if (GqamViewsRepository.General.Properties.repetitions == event.getAffectedEditor()) {
			releaseStep.setRepetitions((EEFConverterUtil.createDoubleFromString(EcorePackage.Literals.EDOUBLE, (String)event.getNewValue())));
		}
		if (GqamViewsRepository.General.Properties.probability == event.getAffectedEditor()) {
			releaseStep.setProbability((EEFConverterUtil.createDoubleFromString(EcorePackage.Literals.EDOUBLE, (String)event.getNewValue())));
		}
		if (GqamViewsRepository.General.Properties.priority == event.getAffectedEditor()) {
			releaseStep.setPriority((EEFConverterUtil.createIntFromString(EcorePackage.Literals.EINT, (String)event.getNewValue())));
		}
		if (GqamViewsRepository.General.Properties.serviceCount == event.getAffectedEditor()) {
			releaseStep.setServiceCount((EEFConverterUtil.createIntFromString(EcorePackage.Literals.EINT, (String)event.getNewValue())));
		}
		if (GqamViewsRepository.General.Properties.resUnits == event.getAffectedEditor()) {
			releaseStep.setResUnits((EEFConverterUtil.createIntFromString(EcorePackage.Literals.EINT, (String)event.getNewValue())));
		}
		if (GqamViewsRepository.General.Properties.relRes == event.getAffectedEditor()) {
			if (event.getKind() == PropertiesEditionEvent.SET) {
				relResSettings.setToReference((Resource)event.getNewValue());
			} else if (event.getKind() == PropertiesEditionEvent.ADD) {
				EReferencePropertiesEditionContext context = new EReferencePropertiesEditionContext(editingContext, this, relResSettings, editingContext.getAdapterFactory());
	...
	public void updatePart(Notification msg) {
		...
		if (editingPart.isVisible()) {
			...
			if (GqamPackage.eINSTANCE.getReleaseStep_ResUnits().equals(msg.getFeature()) && msg.getNotifier().equals(semanticObject) && generalPart != null && isAccessible(GqamViewsRepository.General.Properties.resUnits)) {
				if (msg.getNewValue() != null) {
					generalPart.setResUnits(EcoreUtil.convertToString(EcorePackage.Literals.EINT, msg.getNewValue()));
				} else {
					generalPart.setResUnits("");
				}
			}
			if (GqamPackage.eINSTANCE.getReleaseStep_RelRes().equals(msg.getFeature()) && generalPart != null && isAccessible(GqamViewsRepository.General.Properties.relRes))
				generalPart.setRelRes((EObject)msg.getNewValue());
	...
	protected NotificationFilter[] getNotificationFilters() {
		NotificationFilter filter = new EStructuralFeatureNotificationFilter(
			...
			GqamPackage.eINSTANCE.getReleaseStep_ResUnits(),
			GqamPackage.eINSTANCE.getReleaseStep_RelRes(),
		...
		if (event.getNewValue() != null) {
			try {
				...
				if (GqamViewsRepository.General.Properties.resUnits == event.getAffectedEditor()) {
					Object newValue = event.getNewValue();
					if (newValue instanceof String) {
						newValue = EEFConverterUtil.createFromString(GqamPackage.eINSTANCE.getReleaseStep_ResUnits().getEAttributeType(), (String)newValue);
					}
					ret = Diagnostician.INSTANCE.validate(GqamPackage.eINSTANCE.getReleaseStep_ResUnits().getEAttributeType(), newValue);
				}
