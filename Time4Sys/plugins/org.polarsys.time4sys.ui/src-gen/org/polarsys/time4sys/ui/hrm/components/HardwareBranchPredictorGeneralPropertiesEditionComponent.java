 ...
 * 
 */
public class HardwareBranchPredictorGeneralPropertiesEditionComponent extends SinglePartPropertiesEditingComponent {
	 ...
	 * 
	 */
	public HardwareBranchPredictorGeneralPropertiesEditionComponent(PropertiesEditingContext editingContext, EObject hardwareBranchPredictor, String editing_mode) {
		super(editingContext, hardwareBranchPredictor, editing_mode);
	...
	public void initPart(Object key, int kind, EObject elt, ResourceSet allResource) {
		...
		if (editingPart != null && key == partKey) {
			...
			final HardwareBranchPredictor hardwareBranchPredictor = (HardwareBranchPredictor)elt;
			final GeneralPropertiesEditionPart generalPart = (GeneralPropertiesEditionPart)editingPart;
			// init values
			if (isAccessible(HrmViewsRepository.General.Properties.name))
				generalPart.setName(EEFConverterUtil.convertToString(EcorePackage.Literals.ESTRING, hardwareBranchPredictor.getName()));
			
			if (isAccessible(HrmViewsRepository.General.Properties.ownedResource)) {
				ownedResourceSettings = new ReferencesTableSettings(hardwareBranchPredictor, GrmPackage.eINSTANCE.getResource_OwnedResource());
				generalPart.initOwnedResource(ownedResourceSettings);
			}
			if (isAccessible(HrmViewsRepository.General.Properties.resMult)) {
				generalPart.setResMult(EEFConverterUtil.convertToString(EcorePackage.Literals.EINT, hardwareBranchPredictor.getResMult()));
			}
			
			if (isAccessible(HrmViewsRepository.General.Properties.isProtected)) {
				generalPart.setIsProtected(hardwareBranchPredictor.isIsProtected());
			}
			if (isAccessible(HrmViewsRepository.General.Properties.isActive)) {
				generalPart.setIsActive(hardwareBranchPredictor.isIsActive());
			}
			if (isAccessible(HrmViewsRepository.General.Properties.ownedConnector)) {
				ownedConnectorSettings = new ReferencesTableSettings(hardwareBranchPredictor, GrmPackage.eINSTANCE.getResource_OwnedConnector());
				generalPart.initOwnedConnector(ownedConnectorSettings);
			}
			if (isAccessible(HrmViewsRepository.General.Properties.usedBy)) {
				usedBySettings = new ReferencesTableSettings(hardwareBranchPredictor, GrmPackage.eINSTANCE.getResource_UsedBy());
	...
	public void updateSemanticModel(final IPropertiesEditionEvent event) {
		HardwareBranchPredictor hardwareBranchPredictor = (HardwareBranchPredictor)semanticObject;
		if (HrmViewsRepository.General.Properties.name == event.getAffectedEditor()) {
			hardwareBranchPredictor.setName((java.lang.String)EEFConverterUtil.createFromString(EcorePackage.Literals.ESTRING, (String)event.getNewValue()));
		}
		...
		if (HrmViewsRepository.General.Properties.resMult == event.getAffectedEditor()) {
			hardwareBranchPredictor.setResMult((EEFConverterUtil.createIntFromString(EcorePackage.Literals.EINT, (String)event.getNewValue())));
		}
		if (HrmViewsRepository.General.Properties.isProtected == event.getAffectedEditor()) {
			hardwareBranchPredictor.setIsProtected((Boolean)event.getNewValue());
		}
		if (HrmViewsRepository.General.Properties.isActive == event.getAffectedEditor()) {
			hardwareBranchPredictor.setIsActive((Boolean)event.getNewValue());
		}
	...
	public boolean mustBeComposed(Object key, int kind) {
		return key == HrmViewsRepository.General.Properties.name || key == HrmViewsRepository.General.Properties.ownedResource || key == HrmViewsRepository.General.Properties.resMult || key == HrmViewsRepository.General.Properties.isProtected || key == HrmViewsRepository.General.Properties.isActive || key == HrmViewsRepository.General.Properties.ownedConnector || key == HrmViewsRepository.General.Properties.usedBy || key == HrmViewsRepository.General.Properties.class;
	}
