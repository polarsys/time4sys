 ...
 * 
 */
public class HardwareRomGeneralPropertiesEditionComponent extends SinglePartPropertiesEditingComponent {
	 ...
	 * 
	 */
	public HardwareRomGeneralPropertiesEditionComponent(PropertiesEditingContext editingContext, EObject hardwareRom, String editing_mode) {
	...
	public void initPart(Object key, int kind, EObject elt, ResourceSet allResource) {
		...
		if (editingPart != null && key == partKey) {
			...
			if (isAccessible(HrmViewsRepository.General.Properties.name))
				generalPart.setName(EEFConverterUtil.convertToString(EcorePackage.Literals.ESTRING, hardwareRom.getName()));
			
			if (isAccessible(HrmViewsRepository.General.Properties.ownedResource)) {
				ownedResourceSettings = new ReferencesTableSettings(hardwareRom, GrmPackage.eINSTANCE.getResource_OwnedResource());
				generalPart.initOwnedResource(ownedResourceSettings);
			}
			if (isAccessible(HrmViewsRepository.General.Properties.resMult)) {
				generalPart.setResMult(EEFConverterUtil.convertToString(EcorePackage.Literals.EINT, hardwareRom.getResMult()));
			}
			
			if (isAccessible(HrmViewsRepository.General.Properties.isProtected)) {
				generalPart.setIsProtected(hardwareRom.isIsProtected());
			}
			if (isAccessible(HrmViewsRepository.General.Properties.isActive)) {
				generalPart.setIsActive(hardwareRom.isIsActive());
			}
			if (isAccessible(HrmViewsRepository.General.Properties.ownedConnector)) {
				ownedConnectorSettings = new ReferencesTableSettings(hardwareRom, GrmPackage.eINSTANCE.getResource_OwnedConnector());
				generalPart.initOwnedConnector(ownedConnectorSettings);
			}
			if (isAccessible(HrmViewsRepository.General.Properties.usedBy)) {
				usedBySettings = new ReferencesTableSettings(hardwareRom, GrmPackage.eINSTANCE.getResource_UsedBy());
				generalPart.initUsedBy(usedBySettings);
			}
			if (isAccessible(HrmViewsRepository.General.Properties.memorySize)) {
				generalPart.setMemorySize(EEFConverterUtil.convertToString(EcorePackage.Literals.EINT, hardwareRom.getMemorySize()));
			}
			
			if (isAccessible(HrmViewsRepository.General.Properties.addressSize)) {
				generalPart.setAddressSize(EEFConverterUtil.convertToString(EcorePackage.Literals.EINT, hardwareRom.getAddressSize()));
			}
			
			if (isAccessible(HrmViewsRepository.General.Properties.throughput)) {
				generalPart.setThroughput(EEFConverterUtil.convertToString(EcorePackage.Literals.EINT, hardwareRom.getThroughput()));
			}
			
			if (isAccessible(HrmViewsRepository.General.Properties.type)) {
				generalPart.initType(EEFUtils.choiceOfValues(hardwareRom, HrmPackage.eINSTANCE.getHardwareRom_Type()), hardwareRom.getType());
			}
			// init filters
	...
	public EStructuralFeature associatedFeature(Object editorKey) {
		...
		if (editorKey == HrmViewsRepository.General.Properties.type) {
			return HrmPackage.eINSTANCE.getHardwareRom_Type();
		}
	...
	public void updateSemanticModel(final IPropertiesEditionEvent event) {
		...
		if (HrmViewsRepository.General.Properties.name == event.getAffectedEditor()) {
			hardwareRom.setName((java.lang.String)EEFConverterUtil.createFromString(EcorePackage.Literals.ESTRING, (String)event.getNewValue()));
		}
		...
		if (HrmViewsRepository.General.Properties.resMult == event.getAffectedEditor()) {
			hardwareRom.setResMult((EEFConverterUtil.createIntFromString(EcorePackage.Literals.EINT, (String)event.getNewValue())));
		}
		if (HrmViewsRepository.General.Properties.isProtected == event.getAffectedEditor()) {
			hardwareRom.setIsProtected((Boolean)event.getNewValue());
		}
		if (HrmViewsRepository.General.Properties.isActive == event.getAffectedEditor()) {
			hardwareRom.setIsActive((Boolean)event.getNewValue());
		}
		...
		if (HrmViewsRepository.General.Properties.memorySize == event.getAffectedEditor()) {
			hardwareRom.setMemorySize((EEFConverterUtil.createIntFromString(EcorePackage.Literals.EINT, (String)event.getNewValue())));
		}
		if (HrmViewsRepository.General.Properties.addressSize == event.getAffectedEditor()) {
			hardwareRom.setAddressSize((EEFConverterUtil.createIntFromString(EcorePackage.Literals.EINT, (String)event.getNewValue())));
		}
		if (HrmViewsRepository.General.Properties.throughput == event.getAffectedEditor()) {
			hardwareRom.setThroughput((EEFConverterUtil.createIntFromString(EcorePackage.Literals.EINT, (String)event.getNewValue())));
		}
		if (HrmViewsRepository.General.Properties.type == event.getAffectedEditor()) {
			hardwareRom.setType((RomType)event.getNewValue());
		}
	}
	...
	public void updatePart(Notification msg) {
		...
		if (editingPart.isVisible()) {
			...
			if (HrmPackage.eINSTANCE.getHardwareRom_Type().equals(msg.getFeature()) && msg.getNotifier().equals(semanticObject) && isAccessible(HrmViewsRepository.General.Properties.type))
				generalPart.setType((RomType)msg.getNewValue());
	...
	protected NotificationFilter[] getNotificationFilters() {
		NotificationFilter filter = new EStructuralFeatureNotificationFilter(
			...
			HrmPackage.eINSTANCE.getHardwareRom_Type()		);
	...
	public boolean mustBeComposed(Object key, int kind) {
		return key == HrmViewsRepository.General.Properties.name || key == HrmViewsRepository.General.Properties.ownedResource || key == HrmViewsRepository.General.Properties.resMult || key == HrmViewsRepository.General.Properties.isProtected || key == HrmViewsRepository.General.Properties.isActive || key == HrmViewsRepository.General.Properties.ownedConnector || key == HrmViewsRepository.General.Properties.usedBy || key == HrmViewsRepository.General.Properties.memorySize || key == HrmViewsRepository.General.Properties.addressSize || key == HrmViewsRepository.General.Properties.throughput || key == HrmViewsRepository.General.Properties.type || key == HrmViewsRepository.General.Properties.class;
	}
		...
		if (event.getNewValue() != null) {
			try {
				...
				if (HrmViewsRepository.General.Properties.type == event.getAffectedEditor()) {
					Object newValue = event.getNewValue();
					if (newValue instanceof String) {
						newValue = EEFConverterUtil.createFromString(HrmPackage.eINSTANCE.getHardwareRom_Type().getEAttributeType(), (String)newValue);
					}
					ret = Diagnostician.INSTANCE.validate(HrmPackage.eINSTANCE.getHardwareRom_Type().getEAttributeType(), newValue);
				}
